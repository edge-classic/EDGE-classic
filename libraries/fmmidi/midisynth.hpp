...
class note : uncopyable
{
  public:
    ...
    virtual bool synthesize(int_least32_t *buf, std::size_t samples, float rate, int_least32_t left,
                            int_least32_t right)       = 0;
...
class channel : uncopyable
{
  ...
  public:
    ...
    int  synthesize(int_least32_t *out, std::size_t samples, float rate, int_least32_t master_volume,
                    int master_balance);
...
class fm_operator
{
  public:
    ...
    inline int operator()(int lfo, int m)
    {
        return get_next(lfo, m);
    }
