...
static void makePmTable(void)
{
    ...
    for (i = 0; i < PM_PG_WIDTH; i++)
        /* pmtable[i] = (e_int32) ((double) PM_AMP * pow (2, (double) PM_DEPTH * sin (2.0 * PI * i / PM_PG_WIDTH) /
         * 1200)); */
...
static void makeDphaseTable(void)
{
    ...
    e_uint32 mltable[16] = {1,     1 * 2, 2 * 2,  3 * 2,  4 * 2,  5 * 2,  6 * 2,  7 * 2,
...
{
    dump[0] =
        (e_uint8)((patch[0].AM << 7) + (patch[0].PM << 6) + (patch[0].EG << 5) + (patch[0].KR << 4) + patch[0].ML);
    dump[1] =
        (e_uint8)((patch[1].AM << 7) + (patch[1].PM << 6) + (patch[1].EG << 5) + (patch[1].KR << 4) + patch[1].ML);
...
#define UPDATE_TLL(S)                                                                                                  \
    (((S)->type == 0) ? ((S)->tll = tllTable[((S)->fnum) >> 5][(S)->block][(S)->patch->TL][(S)->patch->KL])            \
...
INLINE static void keyOn_BD(OPLL *opll)
{
    keyOn(opll, 6);
}
...
/* Drum key off */
INLINE static void keyOff_BD(OPLL *opll)
{
    keyOff(opll, 6);
}
...
/* PG */
INLINE static void calc_phase(OPLL_SLOT *slot, e_int32 lfo)
{
    if (slot->patch->PM)
...
/* EG */
static void calc_envelope(OPLL_SLOT *slot, e_int32 lfo)
{
#define S2E(x) (SL2EG((e_int32)(x / SL_STEP)) << (EG_DP_BITS - EG_BITS))

    static e_uint32 SL[16] = {S2E(0.0),  S2E(3.0),  S2E(6.0),  S2E(9.0),  S2E(12.0), S2E(15.0), S2E(18.0), S2E(21.0),
...
/*
  TOP-CYM
 */
INLINE static e_int32 calc_slot_cym(OPLL_SLOT *slot, e_uint32 pgout_hh)
{
    e_uint32 dbout;
...
/*
  HI-HAT
*/
INLINE static e_int32 calc_slot_hat(OPLL_SLOT *slot, e_int32 pgout_cym, e_uint32 noise)
{
    e_uint32 dbout;
...
e_int16 OPLL_calc(OPLL *opll)
{
    ...
    opll->out =
        (e_int16)(((double)opll->next * (opll->opllstep - opll->oplltime) + (double)opll->prev * opll->oplltime) /
                  opll->opllstep);
