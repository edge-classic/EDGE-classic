...
static const unsigned voice_num[18] = {0x0,   0x1,   0x2,   0x3,   0x4,   0x5,   0x6,   0x7,   0x8,
                                       0x100, 0x101, 0x102, 0x103, 0x104, 0x105, 0x106, 0x107, 0x108};

static const unsigned oper_num[18] = {0x0,   0x1,   0x2,   0x8,   0x9,   0xA,   0x10,  0x11,  0x12,
                                      0x100, 0x101, 0x102, 0x108, 0x109, 0x10A, 0x110, 0x111, 0x112};

// ----------------------------------------------------------------------------
OPLPlayer::OPLPlayer(int frequency)
{
    m_voices.resize(18);
...
// ----------------------------------------------------------------------------
void OPLPlayer::loadDefaultPatches()
{
	OPLPatch::loadDefault(m_patches);
}
...
void OPLPlayer::runSamples(unsigned count)
{
    ...
    // so that opal can reassign operators to channels, etc
...
void OPLPlayer::updateVolume(OPLVoice &voice)
{
    ...
    static const uint8_t opl_volume_map[32] = {80, 63, 40, 36, 32, 28, 23, 21, 19, 17, 15, 14, 13, 12, 11, 10,
...
void OPLPlayer::midiNoteOn(uint8_t channel, uint8_t note, uint8_t velocity)
{
    ...
    for (int i = 0; i < numVoices; i++)
    {
        ...
        voice->note                    = note;
        voice->velocity                = (((int)velocity + newPatch->velocity) & 127);
        voice->duration                = 0;
