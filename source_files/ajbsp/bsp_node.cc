...
{
  for (size_t i = 0; i < alloc_cuts.size(); i++) delete alloc_cuts[i];
...
void Seg::Recompute()
{
  ...
  if (p_length_ <= 0) I_Error("AJBSP: Seg %p has zero p_length_.\n", this);
...
#if DEBUG_SPLIT
  if (old_seg->linedef)
    I_Debugf("Splitting Linedef %d (%p) at (%1.1f,%1.1f)\n",
             old_seg->linedef->index, old_seg, x, y);
  else
    I_Debugf("Splitting Miniseg %p at (%1.1f,%1.1f)\n", old_seg, x, y);
#endif
...
#if DEBUG_SPLIT
  I_Debugf("Splitting Vertex is %04X at (%1.1f,%1.1f)\n", new_vert->index,
...
//
inline void ComputeIntersection(Seg *seg, Seg *part, double perp_c,
...
void AddIntersection(Intersection **cut_list, Vertex *vert, Seg *part,
                     bool self_ref)
{
  ...
  {
    if (vert->Overlaps(cut->vertex)) return;
  }
  ...
  while (after && cut->along_dist < after->along_dist) after = after->prev;
                         ...
                         eval_info_t *info)
{
  ...
  for (Seg *check = tree->list_; check; check = check->next_)
  {
    ...
    if (a > -kEpsilon && b > -kEpsilon)
    {
      ...
      if ((a >= kIffySegLength && b >= kIffySegLength) ||
          (a <= kEpsilon && b >= kIffySegLength) ||
...
double EvalPartition(QuadTree *tree, Seg *part, double best_cost)
{
  ...
  if (EvalPartitionWorker(tree, part, best_cost, &info)) return -1.0;

  /* make sure there is at least one real seg on each side */
  if (info.real_left == 0 || info.real_right == 0)
  {
#if DEBUG_PICKNODE
    I_Debugf("Eval : No real segs on %s%sside\n", info.real_left ? "" : "left ",
...
void EvaluateFastWorker(QuadTree *tree, Seg **best_H, Seg **best_V, int mid_x,
                        int mid_y)
{
...
Seg *FindFastSeg(QuadTree *tree)
{
  ...
  if (best_H) H_cost = EvalPartition(tree, best_H, 1.0e99);
...
bool PickNodeWorker(QuadTree *part_list, QuadTree *tree, Seg **best,
                    double *best_cost)
{
  ...
  {
#if DEBUG_PICKNODE
    I_Debugf("PickNode:   %sSEG %p  (%1.1f,%1.1f) -> (%1.1f,%1.1f)\n",
             part->linedef ? "" : "MINI", part, part->start->x, part->start->y,
...
static Seg *PickNode(QuadTree *tree)
{
...
#if DEBUG_PICKNODE
  I_Debugf("PickNode: BEGUN (depth %d)\n", depth);
#endif
  ...
  if (tree->real_num_ >= kSegFastModeThreshold)
  {
    ...
    {
#if DEBUG_PICKNODE
      I_Debugf("PickNode: Using Fast node (%1.1f,%1.1f) -> (%1.1f,%1.1f)\n",
...
//       or be split.
//
static void DivideOneSeg(Seg *seg, Seg *part, Seg **left_list, Seg **right_list,
                         Intersection **cut_list)
{
  ...
  if (seg->source_line_ == part->source_line_) a = b = 0;
...
static void AddMinisegs(Intersection *cut_list, Seg *part, Seg **left_list,
                        Seg **right_list)
{
...
#if DEBUG_CUTLIST
  I_Debugf("CUT LIST:\n");
  I_Debugf("PARTITION: (%1.1f,%1.1f) += (%1.1f,%1.1f)\n", part->psx_,
           part->psy_, part->pdx_, part->pdy_);

  for (cut = cut_list; cut; cut = cut->next)
  {
    I_Debugf("  Vertex %8X (%1.1f,%1.1f)  Along %1.2f  [%d/%d]  %s\n",
             cut->vertex->index, cut->vertex->x, cut->vertex->y,
             cut->along_dist, cut->open_before ? 1 : 0, cut->open_after ? 1 : 0,
  ...
  for (cut = cut_list; cut && cut->next; cut = cut->next)
  {
    ...
    if (len < -0.001)
    {
      I_Error("AJBSP: Bad order in intersect list: %1.3f > %1.3f\n",
...
#if DEBUG_CUTLIST
    I_Debugf("AddMiniseg: %p RIGHT  (%1.1f,%1.1f) -> (%1.1f,%1.1f)\n",
             seg->start_->x, seg->start_->y, seg->end_->x, seg->end_->y);

    I_Debugf("AddMiniseg: %p LEFT   (%1.1f,%1.1f) -> (%1.1f,%1.1f)\n",
...
int Seg::PointOnLineSide(double x, double y) const
{
  ...
  if (fabs(perp) <= kEpsilon) return 0;
...
Seg *CreateOneSeg(Linedef *line, Vertex *start, Vertex *end, Sidedef *side,
                  int what_side)
{
  Seg *seg = NewSeg();
  ...
  if (start->overlap_) start = start->overlap_;
  if (end->overlap_) end = end->overlap_;
...
Seg *CreateSegs()
{
  ...
  for (int i = 0; i < level_linedefs.size(); i++)
  {
    ...
    {
      I_Printf("Linedef #%d is VERY long, it may cause problems\n",
               line->index);
      current_build_info.total_warnings++;
    }
    ...
    {
      I_Printf("Linedef #%d has no right sidedef!\n", line->index);
      current_build_info.total_warnings++;
    }
    ...
    else
    {
      if (line->two_sided)
      {
        I_Printf("Linedef #%d is 2s but has no left sidedef\n", line->index);
        current_build_info.total_warnings++;
...
void Subsector::ClockwiseOrder()
{
  ...
  for (seg = seg_list_; seg; seg = seg->next_)
  {
    // compute angles now
    seg->cmp_angle_ =
        ComputeAngle(seg->start_->x_ - mid_x_, seg->start_->y_ - mid_y_);
...
void Subsector::SanityCheckClosed() const
{
  ...
  for (seg = seg_list_; seg; seg = seg->next_)
  {
    ...
    if (fabs(dx) > kEpsilon || fabs(dy) > kEpsilon) gaps++;
  ...
  if (gaps > 0)
  {
    ...
    for (seg = seg_list; seg; seg = seg->next)
    {
      I_Debugf("  SEG %p  (%1.1f,%1.1f) --> (%1.1f,%1.1f)\n", seg,
               seg->start_->x, seg->start_->y, seg->end_->x, seg->end_->y);
    }
#endif
  }
...
#if DEBUG_SUBSEC
    I_Debugf("Subsec:   %d: Seg %p  Index %d\n", seg_count, seg, seg->index);
#endif
  }
}
...
void DebugShowSegs(const Seg *list)
{
  ...
  {
    I_Debugf("Build:   %sSEG %p  (%1.1f,%1.1f) -> (%1.1f,%1.1f)\n",
             seg->linedef ? "" : "MINI", seg, seg->start_->x, seg->start_->y,
                       ...
                       Node **N, Subsector **S)
{
...
#if DEBUG_BUILDER
  I_Debugf("Build: PARTITION %p (%1.0f,%1.0f) -> (%1.0f,%1.0f)\n", part,
  ...
  *N         = node;
  ...
  if (lefts == NULL) I_Error("AJBSP: Separated seg-list has empty LEFT side\n");
...
#if DEBUG_BUILDER
  I_Debugf("Build: Going LEFT\n");
#endif

  BuildResult ret;

  // recursively build the left side
  ret = BuildNodes(lefts, depth + 1, &node->l_.bounds, &node->l_.node,
                   &node->l_.subsec);
  if (ret != kBuildOK) return ret;

#if DEBUG_BUILDER
  I_Debugf("Build: Going RIGHT\n");
#endif

  // recursively build the right side
  ret = BuildNodes(rights, depth + 1, &node->r_.bounds, &node->r_.node,
                   &node->r_.subsec);
  if (ret != kBuildOK) return ret;
