...
void Seg::Recompute()
{
    ...
    if (p_length_ <= 0)
        I_Error("AJBSP: Seg %p has zero p_length_.\n", this);
...
//
bool EvalPartitionWorker(QuadTree *tree, Seg *part, double best_cost, eval_info_t *info)
{
...
QuadTree::QuadTree(int x1, int y1, int x2, int y2)
    : x1_(x1), y1_(y1), x2_(x2), y2_(y2), real_num_(0), mini_num_(0), list_(NULL)
{
    int dx = x2 - x1;
    int dy = y2 - y1;
...
void Subsector::SanityCheckClosed() const
{
    ...
    if (gaps > 0)
    {
        I_Printf("Subsector #%d near (%1.1f,%1.1f) is not closed "
                   "(%d gaps, %d segs)\n",
...
void Subsector::Normalise()
{
    ...
    while (seg_list_)
    {
        ...
        {
#if DEBUG_SUBSEC
            I_Debugf("Subsec: Removing miniseg %p\n", seg);
#endif
...
void Subsector::RoundOff()
{
...
#if DEBUG_SUBSEC
    I_Debugf("Subsec: Rounding off %d\n", index);
#endif
    ...
    for (seg = seg_list_; seg; seg = seg->next_)
    {
        ...
        if (I_ROUND(seg->start_->x_) == I_ROUND(seg->end_->x_) && I_ROUND(seg->start_->y_) == I_ROUND(seg->end_->y_))
        {
...
#if DEBUG_SUBSEC
    I_Debugf("Subsec: degen=%d real=%d\n", degen_total, real_total);
#endif
    ...
    if (real_total == 0)
    {
        if (last_real_degen == NULL)
            I_Error("AJBSP: Subsector %d rounded off with NO real segs\n", index_);

#if DEBUG_SUBSEC
        I_Debugf("Degenerate before: (%1.2f,%1.2f) -> (%1.2f,%1.2f)\n", last_real_degen->start->x,
...
#if DEBUG_SUBSEC
        I_Debugf("Degenerate after:  (%d,%d) -> (%d,%d)\n", I_ROUND(last_real_degen->start->x),
    ...
    while (seg_list_ != NULL)
    {
        ...
        {
#if DEBUG_SUBSEC
            I_Debugf("Subsec: Removing degenerate %p\n", seg);
#endif
            // this causes SortSegs() to remove the seg
            seg->index_ = kSegIsGarbage;
            continue;
    ...
    if (new_head == NULL)
        I_Error("AJBSP: Subsector %d rounded off to being EMPTY\n", index_);
