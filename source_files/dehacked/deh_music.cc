...
void music::AlterBexMusic(const char *new_val)
{
    ...
    {
        EDGEDebugf("Dehacked: Warning - Bad length for music name '%s'.\n",
                 new_val);
        return;
    }
    ...
    if (epi::IsDigitASCII(old_val[0]))
    {
        ...
        if (num < 1 || num > 32767)
        {
            EDGEDebugf("Dehacked: Warning - Line %d: illegal music entry '%s'.\n",
    ...
    if (!ReplaceMusic(old_val, new_val))
        EDGEDebugf("Dehacked: Warning - Line %d: unknown music name '%s'.\n",
                 patch::line_num, old_val);
}
