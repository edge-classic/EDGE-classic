...
static void NAV_EnemiesInNode(unsigned int bspnum, bot_t *bot, float radius, mobj_t *&best_mo, float &best_score)
{
    ...
    for (int c = 0; c < 2; c++)
    {
        ...
        if (node->bbox[c][BOXLEFT] > pos.x + radius)
            continue;
        if (node->bbox[c][BOXRIGHT] < pos.x - radius)
            continue;
        if (node->bbox[c][BOXBOTTOM] > pos.y + radius)
            continue;
        if (node->bbox[c][BOXTOP] < pos.y - radius)
            continue;
...
bam_angle bot_path_c::cur_angle() const
{
    position_c src  = nodes.at(along - 1).pos;
    position_c dest = nodes.at(along + 0).pos;
