...
void HUD_DrawImageTitleWS(const image_c *title_image)
{
    ...
    if (r_titlescaling.d ==
        1) // Prserve aspect ratio; scale until vertical and horizontal screen space is filled ("Zoom")
    {
...
void HUD_DrawChar(float left_x, float top_y, const image_c *img, char ch, float size)
{
    ...
    else
    {
        ...
        h = (size > 0 ? size : (cur_font->def->default_size > 0.0 ? cur_font->def->default_size : IM_HEIGHT(img))) *
            sc_y;
        tx1 = 0;
        ty1 = 0;
        tx2 = IM_RIGHT(img);
