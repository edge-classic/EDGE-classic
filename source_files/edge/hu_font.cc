...
font_c::font_c(FontDefinition *_def) : def(_def)
{
    font_image = nullptr;
    ttf_info   = nullptr;
    ttf_buffer = nullptr;
...
void font_c::LoadPatches()
{
    ...
    for (pat = def->patches_; pat; pat = pat->next)
    {
        if (pat->char1 < first)
            first = pat->char1;
	...
	if (def->default_size_ > 0.0)
	{
		p_cache.height = def->default_size_;
		p_cache.width  = def->default_size_ * (Nom.iw / Nom.ih);
...
void font_c::LoadFontImage()
{
    if (!font_image)
    {
        ...
        if (!font_image)
            I_Error("LoadFontImage: Image %s not found for font %s!", def->image_name_.c_str(), def->name_.c_str());
        int char_height = font_image->actual_h / 16;
        int char_width  = font_image->actual_w / 16;
        im_char_height  = (def->default_size_ == 0.0 ? char_height : def->default_size_) * font_image->scale_y;
        im_char_width   = (def->default_size_ == 0.0 ? char_width : def->default_size_) * font_image->scale_x;
        im_mono_width   = 0;
        spacing         = def->spacing_;
        ...
        for (int i = 0; i < 256; i++)
        {
            ...
            individual_char_widths[i] =
                ...
                font_image->scale_x;
            if (def->default_size_ > 0.0)
                individual_char_widths[i] *= (def->default_size_ / char_width);
...
void font_c::LoadFontTTF()
{
    if (!ttf_buffer)
    {
        if (def->ttf_name_.empty())
        {
            I_Error("LoadFontTTF: No TTF file/lump name provided for font %s!", def->name_.c_str());
        }

        for (size_t i = 0; i < hu_fonts.size(); i++)
        {
            if (epi::StringCaseCompareASCII(hu_fonts[i]->def->ttf_name_, def->ttf_name_) == 0)
            {
        ...
        if (!ttf_buffer)
        {
            ...
            if (!epi::GetExtension(def->ttf_name_).empty()) // check for pack file
                F = W_OpenPackFile(def->ttf_name_);
            else
                F = W_OpenLump(W_CheckNumForName(def->ttf_name_.c_str()));

            if (!F)
                I_Error("LoadFontTTF: '%s' not found for font %s.\n", def->ttf_name_.c_str(), def->name_.c_str());
        ...
        for (int i = 0; i < 3; i++)
        {
            ...
            if (def->default_size_ == 0.0)
                def->default_size_ = 7.0f;

            ttf_kern_scale[i] = stbtt_ScaleForPixelHeight(ttf_info, def->default_size_);
            ...
            ref.width[i]      = (ref.char_quad[i]->x1 - ref.char_quad[i]->x0) * (def->default_size_ / res_font_sizes[i]);
            ref.height[i]     = (ref.char_quad[i]->y1 - ref.char_quad[i]->y0) * (def->default_size_ / res_font_sizes[i]);
            ttf_char_width[i] = ref.width[i];
            ttf_char_height[i] = (ascent - descent) * (def->default_size_ / res_font_sizes[i]);
            ref.y_shift[i] =
                (ttf_char_height[i] - ref.height[i]) + (ref.char_quad[i]->y1 * (def->default_size_ / res_font_sizes[i]));
            ttf_ref_yshift[i] = ref.y_shift[i];
            ttf_ref_height[i] = ref.height[i];
        }
        ttf_glyph_map.try_emplace((uint8_t)ch, ref);
        spacing = def->spacing_ + 0.5; // + 0.5 for at least a minimal buffer between letters by default
    }
}
...
{
    if (def->type_ == kFontTypeImage)
        return im_char_width + spacing;

    if (def->type_ == kFontTypePatch)
        return p_cache.width + spacing;

    if (def->type_ == kFontTypeTrueType)
...
const image_c *font_c::CharImage(char ch) const
{
    ...
    SYS_ASSERT(def->type_ == kFontTypePatch);
...
{
    SYS_ASSERT(def->type_ == kFontTypeImage);
...
float font_c::CharWidth(char ch)
{
            ...
            for (int i = 0; i < 3; i++)
            {
                ...
                if (ch == ' ')
                    ...
                    character.width[i] = (character.char_quad[i]->x1 - character.char_quad[i]->x0) *
                                         (def->default_size_ / res_font_sizes[i]);
                character.height[i] =
                    (character.char_quad[i]->y1 - character.char_quad[i]->y0) * (def->default_size_ / res_font_sizes[i]);
                character.y_shift[i] = (ttf_char_height[i] - character.height[i]) +
                                       (character.char_quad[i]->y1 * (def->default_size_ / res_font_sizes[i]));
            }
    ...
    if (def->default_size_ > 0.0)
        return (def->default_size_ * ((float)rect.iw) / rect.ih) + spacing;
    else
...
{
    assert(def->type_ == kFontTypeTrueType);
    ...
    else
    {
        ...
        for (int i = 0; i < 3; i++)
        {
            ...
            if (ch == ' ')
                ...
                character.width[i] =
                    (character.char_quad[i]->x1 - character.char_quad[i]->x0) * (def->default_size_ / res_font_sizes[i]);
            character.height[i] =
                (character.char_quad[i]->y1 - character.char_quad[i]->y0) * (def->default_size_ / res_font_sizes[i]);
            character.y_shift[i] = (ttf_char_height[i] - character.height[i]) +
                                   (character.char_quad[i]->y1 * (def->default_size_ / res_font_sizes[i]));
...
float font_c::StringWidth(const char *str)
{
    ...
    for (size_t i = 0; i < width_checker.size(); i++)
    {
        ...
        if (def->type_ == kFontTypeTrueType && i + 1 < width_checker.size())
...
//
font_c *font_container_c::Lookup(FontDefinition *def)
{
    SYS_ASSERT(def);
