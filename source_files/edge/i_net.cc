...
const char *net_address_c::TempString(bool with_port) const
{
    ...
    if (with_port)
    {
        sprintf(buffer, "%d.%d.%d.%d:%d", (int)addr[0], (int)addr[1], (int)addr[2], (int)addr[3], port);
    }
    else
    {
        sprintf(buffer, "%d.%d.%d.%d", (int)addr[0], (int)addr[1], (int)addr[2], (int)addr[3]);
    }
...
bool net_address_c::FromString(const char *str)
{
    ...
    if (4 != sscanf(str, " %d.%d.%d.%d  ", tmp_addr + 0, tmp_addr + 1, tmp_addr + 2, tmp_addr + 3))
        return false;
...
static bool GetLocalAddress(void)
{
    ...
    for (int i = 0; local->h_addr_list[i] != NULL; i++)
    {
        ...
        if (best < 0 && raw_addr[0] != 127 && raw_addr[0] != 0 && raw_addr[0] != 255)
        {
            best = i;
        }
    }
...
static bool Scan_IFCONFIG(bool got_local)
{
    ...
    while (offset + (int)sizeof(struct ifreq) <= (int)config.ifc_len)
    {
        ...
        if (req.ifr_addr.sa_family == AF_INET)
        {
            ...
            if (cur_A.addr[0] != 127 && cur_A.addr[0] != 0 && cur_A.addr[0] != 255 &&
...
void I_SetNoDelay(SOCKET sock, bool enable)
{
    ...
    setsockopt(sock, IPPROTO_TCP, TCP_NODELAY, (char *)&mode, sizeof(mode));
#endif
}
...
void I_SetBroadcast(SOCKET sock, bool enable)
{
    ...
    setsockopt(sock, SOL_SOCKET, SO_BROADCAST, (char *)&mode, sizeof(mode));
#endif
}
