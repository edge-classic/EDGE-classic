...
static void HorizontalBlurRgb(uint8_t *in, uint8_t *out, int w, int h, int c,
                              int r)
{
    ...
    for (int i = 0; i < h; i++)
    {
        ...
        int lv[3]  = {in[(ti + w - 1) * c + 0], in[(ti + w - 1) * c + 1],
                      in[(ti + w - 1) * c + 2]};
...
static inline uint32_t Hq2xGetR(uint32_t col) { return (col >> 16) & 0xFF; }
static inline uint32_t Hq2xGetG(uint32_t col) { return (col >> 8) & 0xFF; }
static inline uint32_t Hq2xGetB(uint32_t col) { return (col) & 0xFF; }
static inline uint32_t Hq2xGetA(uint32_t col) { return (col >> 24) & 0xFF; }

static inline void InterpolateColor(uint8_t *dest, uint32_t c1, uint32_t c2,
                                    uint32_t c3, uint32_t f1, uint32_t f2,
                                    uint32_t f3, uint32_t shift)
{
    dest[0] =
        (Hq2xGetR(c1) * f1 + Hq2xGetR(c2) * f2 + Hq2xGetR(c3) * f3) >> shift;
    dest[1] =
        (Hq2xGetG(c1) * f1 + Hq2xGetG(c2) * f2 + Hq2xGetG(c3) * f3) >> shift;
    dest[2] =
        (Hq2xGetB(c1) * f1 + Hq2xGetB(c2) * f2 + Hq2xGetB(c3) * f3) >> shift;
    dest[3] =
        (Hq2xGetA(c1) * f1 + Hq2xGetA(c2) * f2 + Hq2xGetA(c3) * f3) >> shift;
}
...
inline bool YuvDiff(const uint8_t p1, const uint8_t p2)
{
    ...
    return (YUV1 & a_mask) != (YUV2 & a_mask) ||
           HMM_ABS((int)((YUV1 & y_mask) - (YUV2 & y_mask))) > tr_y ||
...
void Hq2xPaletteSetup(const uint8_t *palette, int transparent_pixel)
{
    for (int c = 0; c < 256; c++)
    {
        ...
        if (c == transparent_pixel) r = g = b = A = 0;
...
void ConvertLine(int y, int w, int h, bool invert, uint8_t *dest,
                 const uint8_t *src)
{
    ...
    for (int x = 0; x < w; x++, src++, dest += 8 /* RGBA */)
    {
        ...
        for (int k = 1; k <= 9; k++) c[k] = pixel_rgb[p[k]];

        uint8_t pattern = 0;

        if (YuvDiff(p[5], p[1])) pattern |= 0x01;
        if (YuvDiff(p[5], p[2])) pattern |= 0x02;
        if (YuvDiff(p[5], p[3])) pattern |= 0x04;
        if (YuvDiff(p[5], p[4])) pattern |= 0x08;
        if (YuvDiff(p[5], p[6])) pattern |= 0x10;
        if (YuvDiff(p[5], p[7])) pattern |= 0x20;
        if (YuvDiff(p[5], p[8])) pattern |= 0x40;
        if (YuvDiff(p[5], p[9])) pattern |= 0x80;
        ...
        switch (pattern)
        {
            ...
            {
                Interpolate2(dest, c[5], c[1], c[4]);
                if (YuvDiff(p[2], p[6])) { Interpolate1(dest + 4, c[5], c[3]); }
                else { Interpolate2(dest + 4, c[5], c[2], c[6]); }
                Interpolate2(dest + BpL, c[5], c[8], c[4]);
                Interpolate2(dest + BpL + 4, c[5], c[9], c[8]);
                break;
            }
            ...
            case 81:
            {
                ...
                else { Interpolate2(dest + BpL + 4, c[5], c[6], c[8]); }
                break;
            }
            case 72:
            case 76:
            {
                ...
                else { Interpolate2(dest + BpL, c[5], c[8], c[4]); }
                Interpolate2(dest + BpL + 4, c[5], c[9], c[6]);
                break;
            }
            case 10:
            case 138:
            {
                if (YuvDiff(p[4], p[2])) { Interpolate1(dest, c[5], c[1]); }
                else { Interpolate2(dest, c[5], c[4], c[2]); }
                Interpolate2(dest + 4, c[5], c[3], c[6]);
                Interpolate2(dest + BpL, c[5], c[7], c[8]);
                Interpolate2(dest + BpL + 4, c[5], c[6], c[8]);
                break;
            ...
            {
                Interpolate2(dest, c[5], c[1], c[4]);
                if (YuvDiff(p[2], p[6])) { Interpolate0(dest + 4, c[5]); }
                else { Interpolate2(dest + 4, c[5], c[2], c[6]); }
                Interpolate2(dest + BpL, c[5], c[8], c[4]);
                Interpolate2(dest + BpL + 4, c[5], c[9], c[8]);
                break;
            ...
            case 209:
            {
                ...
                Interpolate2(dest + 4, c[5], c[3], c[2]);
                Interpolate2(dest + BpL, c[5], c[7], c[4]);
                if (YuvDiff(p[6], p[8])) { Interpolate0(dest + BpL + 4, c[5]); }
                else { Interpolate2(dest + BpL + 4, c[5], c[6], c[8]); }
                break;
            }
            case 104:
            case 108:
            {
                Interpolate2(dest, c[5], c[1], c[2]);
                Interpolate2(dest + 4, c[5], c[2], c[6]);
                if (YuvDiff(p[8], p[4])) { Interpolate0(dest + BpL, c[5]); }
                else { Interpolate2(dest + BpL, c[5], c[8], c[4]); }
                Interpolate2(dest + BpL + 4, c[5], c[9], c[6]);
                break;
            }
            ...
            case 139:
            {
                if (YuvDiff(p[4], p[2])) { Interpolate0(dest, c[5]); }
                else { Interpolate2(dest, c[5], c[4], c[2]); }
                Interpolate2(dest + 4, c[5], c[3], c[6]);
                Interpolate2(dest + BpL, c[5], c[7], c[8]);
                Interpolate2(dest + BpL + 4, c[5], c[6], c[8]);
            ...
            {
                if (YuvDiff(p[4], p[2])) { Interpolate1(dest, c[5], c[1]); }
                else { Interpolate7(dest, c[5], c[4], c[2]); }
                if (YuvDiff(p[2], p[6])) { Interpolate1(dest + 4, c[5], c[3]); }
                else { Interpolate7(dest + 4, c[5], c[2], c[6]); }
                Interpolate1(dest + BpL, c[5], c[8]);
                Interpolate2(dest + BpL + 4, c[5], c[9], c[8]);
                break;
            }
            case 83:
            {
                ...
                else { Interpolate7(dest + BpL + 4, c[5], c[6], c[8]); }
                break;
            }
            case 92:
            {
                ...
                else { Interpolate7(dest + BpL, c[5], c[8], c[4]); }
                if (YuvDiff(p[6], p[8]))
                {
                    Interpolate1(dest + BpL + 4, c[5], c[9]);
                }
            ...
            case 233:
            {
                ...
                Interpolate2(dest + 4, c[5], c[2], c[6]);
                if (YuvDiff(p[8], p[4])) { Interpolate0(dest + BpL, c[5]); }
                else { Interpolate10(dest + BpL, c[5], c[8], c[4]); }
                Interpolate1(dest + BpL + 4, c[5], c[6]);
                break;
            }
            case 175:
            ...
            {
                if (YuvDiff(p[4], p[2])) { Interpolate0(dest, c[5]); }
                else { Interpolate10(dest, c[5], c[4], c[2]); }
                Interpolate1(dest + 4, c[5], c[6]);
                Interpolate1(dest + BpL, c[5], c[8]);
                Interpolate2(dest + BpL + 4, c[5], c[6], c[8]);
                break;
            ...
            case 151:
            {
                Interpolate1(dest, c[5], c[4]);
                if (YuvDiff(p[2], p[6])) { Interpolate0(dest + 4, c[5]); }
                else { Interpolate10(dest + 4, c[5], c[2], c[6]); }
                Interpolate2(dest + BpL, c[5], c[8], c[4]);
                Interpolate1(dest + BpL + 4, c[5], c[8]);
                break;
            }
            ...
            case 244:
            {
                ...
                Interpolate1(dest + BpL, c[5], c[4]);
                if (YuvDiff(p[6], p[8])) { Interpolate0(dest + BpL + 4, c[5]); }
                else { Interpolate10(dest + BpL + 4, c[5], c[6], c[8]); }
                break;
            }
            case 250:
            {
...
ImageData *ImageHq2x(ImageData *image, bool solid, bool invert)
{
    ...
    if (solid) temp_buffer = new uint8_t[w * 16];  // two lines worth
