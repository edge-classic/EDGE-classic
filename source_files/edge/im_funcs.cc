...
ImageFormat ImageDetectFormat(uint8_t *header, int header_length, int file_size)
{
    ...
    if (header_length < 12) return kImageUnknown;
    ...
    if (header[0] == 'D' && header[1] == 'D' && header[2] == 'S' &&
        header[3] == 0x20 && header[4] == 124 && header[5] == 0 &&
        header[6] == 0)
    {
...

    {
        ...
        if (width > 0 && width <= 4096 && abs(ofs_x) <= 4096 && height > 0 &&
            height <= 1024 && abs(ofs_y) <= 4096 &&
...
ImageFormat ImageFormatFromFilename(const std::string &filename)
{
    ...
    if (ext == ".png") return kImagePng;

    if (ext == ".tga") return kImageTga;

    if (ext == ".jpg" || ext == ".jpeg") return kImageJpeg;

    if (ext == ".lmp")  // Kind of a gamble, but whatever
        return kImageDoom;

    if (ext == ".gif" || ext == ".bmp" || ext == ".dds") return kImageOther;
...
ImageData *ImageLoad(epi::File *file)
{
    ...
    uint8_t *decoded_img =
        stbi_load_from_memory(raw_image, length, &width, &height, &depth_, 0);
    ...
    if (decoded_img != nullptr && (depth_ == 1 || depth_ == 2))
    {
        ...
        decoded_img = stbi_load_from_memory(raw_image, length, &width, &height,
                                            &depth_, new_depth_);
    ...
    if (decoded_img == nullptr) return nullptr;
    ...
    {
        total_w = 1;
        while (total_w < (int)width) total_w <<= 1;
        total_h = 1;
        while (total_h < (int)height) total_h <<= 1;
    }
    ...
    const std::unordered_map<int, ImageData *> &image_pack_data)
{
    ...
    if (atlas_h < atlas_w) atlas_h = atlas_w;
    stbrp_context ctx;
    ...
    while (packres != 1)
    {
        ...
        if (atlas_w > 4096 || atlas_h > 4096)
            FatalError(
                "ImagePack: Atlas exceeds maximum allowed texture size "
                "(4096x4096)!");
        stbrp_init_target(&ctx, atlas_w, atlas_h, nodes, 4096);
    ...
    for (size_t i = 0; i < rects.size(); i++)
    {
        ...
        for (short x = 0; x < im->used_width_; x++)
        {
            ...
            {
                memcpy(atlas->data_->PixelAt(rect_x + x, rect_y + y),
                       im->PixelAt(x, y), 4);
            }
        }
        ...
        atlas_rect.texture_coordinate_height =
            (float)im->used_height_ / atlas_h;
...
bool ImageSaveJpeg(std::string filename, ImageData *image)
{
    ...
    int result = stbi_write_jpg_to_func(StbImageEpiFileWrite, dest,
                                        image->used_width_, image->used_height_,
...
bool ImageSavePng(std::string filename, ImageData *image)
{
    ...
    // zero means failure here
    int result = stbi_write_png_to_func(StbImageEpiFileWrite, dest,
                                        image->used_width_, image->used_height_,
