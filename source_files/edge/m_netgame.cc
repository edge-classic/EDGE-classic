...
static void CreateHostWelcome(welcome_proto_t *we)
{
    ...
    we->gameplay = welcome_proto_t::GP_True3D | welcome_proto_t::GP_MLook;
...
#define HANDLE_FLAG(field, testbit) (flags.field) = (we->gameplay & (testbit)) ? true : false;
    ...
    flags.autoaim = (we->gameplay & welcome_proto_t::GP_AutoAim) ? AA_ON : AA_OFF;
...
static void DrawKeyword(int index, style_c *style, int y, const char *keyword, const char *value)
{
    ...
    bool is_selected = (netgame_menuon == 1 && index == host_pos) || (netgame_menuon == 2 && index == join_pos);
...
void M_DrawJoinMenu(void)
{
    ...
    DrawKeyword(idx, ng_join_style, y, "HOST ADDRESS", join_addr ? join_addr->TempString(false) : "???");
    y += 10;
    idx++;

    DrawKeyword(idx, ng_join_style, y, "HOST PORT", epi::STR_Format("%d", joining_port).c_str());
    y += 20;
    idx++;
...
bool M_NetGameResponder(event_t *ev, int ch)
{
    ...
    case 2:
        return M_NetJoinResponder(ev, ch);
    case 3:
