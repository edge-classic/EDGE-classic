                                 ...
                                 float ceiling_height, MapSurface *ceil, Sector *sec_check, Line *ld)
{
    ...
    // Spawn bullet puff
    if (shoot_check.puff)
        SpawnPuff(x, y, z, shoot_check.puff, shoot_check.angle + kBAMAngle180, false);
...
static bool ShootTraverseCallback(PathIntercept *in, void *dataptr)
{
    ...
    if (in->line)
    {
        ...
        if (shoot_check.puff)
        {
            float puff_x = x - trace.delta_x * 6.0f / shoot_check.range;
            float puff_y = y - trace.delta_y * 6.0f / shoot_check.range;
