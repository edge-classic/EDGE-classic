...
bool P_CheckAbsPosition(mobj_t *thing, float x, float y, float z)
{
    ...
    tm_I.bbox[kBoundingBoxRight]  = x + r;
    tm_I.bbox[kBoundingBoxTop]    = y + r;
                                 ...
                                 sector_t *sec_check, line_t *ld)
{
    ...
    if (sec_check && sec_check->floor_vertex_slope)
    {
        if (sec_check->floor_vs_hilo.X > sec_check->f_h)
        {
            ...
            else
            {
                ...
                {
                    if (MathPointInTriangle({{tri_v1.X, tri_v1.Z}}, {{tri_v2.X, tri_v2.Z}},
                ...
                {
                    if (MathPointInTriangle({{tri_v1.Y, tri_v1.Z}}, {{tri_v2.Y, tri_v2.Z}},
    ...
    else
    {
        if (sec_check && sec_check->floor_vertex_slope)
        {
            ...
            else if (sec_check->ceil_vertex_slope)
            {
                ...
                shoota = MathLinePlaneIntersection({{shoot_I.source->x, shoot_I.source->y, shoot_I.start_z}}, {{sx, sy, z}},
        ...
        {
            // Check ceiling vertex slope intersect from shooter's angle
            HMM_Vec3    shoota     = MathLinePlaneIntersection({{shoot_I.source->x, shoot_I.source->y, shoot_I.start_z}},
    ...
    if (last_shoota_sec && (last_shoota_sec->floor_vertex_slope || last_shoota_sec->ceil_vertex_slope))
    {
        ...
        {
            if (z <= MathLinePlaneIntersection({{x, y, -40000}}, {{x, y, 40000}}, last_shoota_sec->floor_z_verts[2],
        ...
        {
            if (z >= MathLinePlaneIntersection({{x, y, -40000}}, {{x, y, 40000}}, last_shoota_sec->ceil_z_verts[2],
...
static bool PIT_CheckBlockingLine(line_t *line, void *data)
{
    ...
    if (slope_sec && (slope_sec->floor_vertex_slope || slope_sec->ceil_vertex_slope))
    {
        ...
        {
            if (mb2 <= MathLinePlaneIntersection({{mx2, my2, -40000}}, {{mx2, my2, 40000}}, slope_sec->floor_z_verts[2],
        ...
        {
            if (mt2 >= MathLinePlaneIntersection({{mx2, my2, -40000}}, {{mx2, my2, 40000}}, slope_sec->ceil_z_verts[2],
