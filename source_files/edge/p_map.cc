...
EDGE_DEFINE_CONSOLE_VARIABLE_WITH_CALLBACK_CLAMPED(gore_level, "0",
                                                   kConsoleVariableFlagArchive,
                                                   GoreSettingCallback, 0, 2)
...
{
    if (!(thing->flags_ & kMapObjectFlagShootable)) return true;

    // check we aren't trying to stomp ourselves
    if (thing == move_check.mover) return true;

    // ignore old avatars (for Hub reloads), which get removed after loading
    if (thing->hyper_flags_ & kHyperFlagRememberOldAvatars) return true;

    float blockdist = thing->radius_ + move_check.mover->radius_;

    // check to see we hit it
    if (fabs(thing->x - move_check.x) >= blockdist ||
    ...
    if (level_flags.true3dgameplay)
    {
        if (move_check.z >= thing->z + thing->height_)
        {
            // went over
            move_check.floor_z =
                HMM_MAX(move_check.floor_z, thing->z + thing->height_);
            return true;
        }
...
static bool CheckAbsoluteLineCallback(line_t *ld, void *data)
{
    if (BoxOnLineSide(move_check.bounding_box, ld) != -1) return true;

    // The spawning thing's position touches the given line.
    // If this should not be allowed, return false.

    if (move_check.mover->player_ && ld->special &&
        (ld->special->portal_effect_ & kPortalEffectTypeStandard))
        return true;

    if (!ld->backsector || ld->gap_num == 0) return false;  // one sided line
    ...
    for (int i = 0; i < ld->gap_num; i++)
    {
        ...
        {
            if (move_check.mover->height_ <= (ld->gaps[i].c - ld->gaps[i].f))
                return true;
        }
        else
        {
            if (ld->gaps[i].f <= move_check.z &&
                move_check.z + move_check.mover->height_ <= ld->gaps[i].c)
                return true;
        }
    }
...
static bool CheckAbsoluteThingCallback(MapObject *thing, void *data)
{
    ...
    if (move_check.flags & kMapObjectFlagMissile)
    {
        ...
        if ((thing->hyper_flags_ & kHyperFlagMissilesPassThrough) &&
            level_flags.pass_missile)
            return true;
    ...
    return !solid || (thing->flags_ & kMapObjectFlagNoClip) ||
...
{
    // can go anywhere
    if (thing->flags_ & kMapObjectFlagNoClip) return true;
    ...
    if (!BlockmapThingIterator(x - r, y - r, x + r, y + r,
                               CheckAbsoluteThingCallback))
        return false;

    // check lines

    if (!BlockmapLineIterator(x - r, y - r, x + r, y + r,
                              CheckAbsoluteLineCallback))
        return false;
...
static bool CheckRelativeLineCallback(line_t *ld, void *data)
{
    ...
    if (move_check.mover->player_ &&
        !AlmostEquals(ld->frontsector->f_h, ld->backsector->f_h))
    {
        if ((move_check.mover->z < ld->frontsector->f_h &&
             SectorIsLowering(ld->frontsector)) ||
            (move_check.mover->z < ld->backsector->f_h &&
             SectorIsLowering(ld->backsector)))
        {
            block_line = ld;
            return false;
        }
    ...
    if (ld->special) special_lines_hit.push_back(ld);
    ...
    // check for hitting a sky-hack line
    {
        ...
        if (!AlmostEquals(c1, c2) && IS_SKY(ld->frontsector->ceil) &&
            IS_SKY(ld->backsector->ceil) && move_check.z > HMM_MIN(c1, c2))
        {
    ...
    if (ld->frontsector->floor_vertex_slope ||
        ld->backsector->floor_vertex_slope)
    {
        divline_t divver;
        divver.x  = ld->v1->X;
        divver.y  = ld->v1->Y;
        ...
        // parallel to it
        if (PointOnDividingLineThick(move_check.mover->x, move_check.mover->y,
                                     &divver, ld->length,
                                     move_check.mover->radius_) == 2)
            return true;
        if (ld->frontsector->floor_vertex_slope &&
            ld->frontsector->linecount == 4 &&
            R_PointInSubsector(move_check.mover->x, move_check.mover->y)
                    ->sector != ld->frontsector)
        {
            float ix = 0;
            float iy = 0;
            ComputeIntersection(&divver, move_check.mover->x,
                                move_check.mover->y, move_check.x, move_check.y,
                                &ix, &iy);
            if (isfinite(ix) && isfinite(iy))
            {
                iz = MathLinePlaneIntersection(
                         {{ix, iy, -40000}}, {{ix, iy, 40000}},
                         ld->frontsector->floor_z_verts[2],
                         ld->frontsector->floor_vs_normal)
                         .Z;
                if (isfinite(iz) &&
                    iz > move_check.mover->z +
                             move_check.mover->info_->step_size_)
                {
                    block_line = ld;
                    return false;
                }
        ...
        else if (ld->backsector->floor_vertex_slope &&
                 ld->backsector->linecount == 4 &&
                 R_PointInSubsector(move_check.mover->x, move_check.mover->y)
                         ->sector != ld->backsector)
        {
            ...
            if (isfinite(ix) && isfinite(iy))
            {
                iz = MathLinePlaneIntersection({{ix, iy, -40000}},
                                               {{ix, iy, 40000}},
                                               ld->backsector->floor_z_verts[2],
        ...
        else if (ld->frontsector->floor_vertex_slope &&
                 ld->frontsector->linecount == 4 &&
                 R_PointInSubsector(move_check.mover->x, move_check.mover->y)
                         ->sector == ld->frontsector)
        {
            if (!ld->backsector->floor_vertex_slope)
            {
                ...
                if (move_check.mover->z + move_check.mover->info_->step_size_ <
                    iz)
                {
                    block_line = ld;
                    return false;
            ...
            else
            {
                ...
                float iy = 0;
                ComputeIntersection(&divver, move_check.mover->x,
                                    move_check.mover->y, move_check.x,
                                    move_check.y, &ix, &iy);
                if (isfinite(ix) && isfinite(iy))
                {
                    iz = MathLinePlaneIntersection(
                    ...
                    ->sector != ld->frontsector)
        {
            ...
            if (isfinite(ix) && isfinite(iy))
            {
                float icz = MathLinePlaneIntersection(
                                {{ix, iy, -40000}}, {{ix, iy, 40000}},
                                ld->frontsector->ceil_z_verts[2],
        ...
        else if (ld->backsector->ceil_vertex_slope &&
                 ld->backsector->linecount == 4 &&
                 R_PointInSubsector(move_check.mover->x, move_check.mover->y)
                         ->sector != ld->backsector)
        {
        ...
        else if (ld->frontsector->ceil_vertex_slope &&
                 ld->frontsector->linecount == 4 &&
                 R_PointInSubsector(move_check.mover->x, move_check.mover->y)
                         ->sector == ld->frontsector)
        {
            ...
            {
                if (iz + move_check.mover->height_ >= ld->backsector->c_h)
                {
                    block_line = ld;
                    return false;
                }
                         ...
                         ->sector == ld->backsector)
        {
            if (!ld->frontsector->ceil_vertex_slope)
            {
                if (iz + move_check.mover->height_ >= ld->frontsector->c_h)
                {
                    block_line = ld;
                    return false;
                }
    ...
    int i = FindThingGap(ld->gaps, ld->gap_num, move_check.z,
                         move_check.z + move_check.mover->height_);

    // gap has been chosen. apply it.

    if (i >= 0)
    {
        if (ld->gaps[i].f >= move_check.floor_z &&
            !move_check.subsector->sector->floor_vertex_slope)
        {
            move_check.floor_z = ld->gaps[i].f;
            move_check.below   = nullptr;
        }

        if (ld->gaps[i].c < move_check.ceiling_z)
            move_check.ceiling_z = ld->gaps[i].c;

        if (ld->gaps[i].f < move_check.dropoff)
            move_check.dropoff = ld->gaps[i].f;
    }
    else { move_check.ceiling_z = move_check.floor_z; }
    ...
    if (!block_line)
    {
        if (move_check.line_count == 0) move_check.line_which = ld;
...
static bool CheckRelativeThingCallback(MapObject *thing, void *data)
{
    ...
    if (0 == (thing->flags_ & (kMapObjectFlagSolid | kMapObjectFlagSpecial |
                               kMapObjectFlagShootable | kMapObjectFlagTouchy)))
        return true;
    ...
    if (level_flags.true3dgameplay && !(thing->flags_ & kMapObjectFlagSpecial))
    {
        ...
        if (move_check.z >= top_z)
        {
            if (top_z > move_check.floor_z &&
                !(thing->flags_ & kMapObjectFlagMissile))
            {
        ...
        if (top_z > move_check.floor_z &&
            (thing->extended_flags_ & kExtendedFlagClimbable) &&
            (move_check.mover->player_ ||
             (move_check.extended_flags & kExtendedFlagMonster)) &&
            ((move_check.flags & kMapObjectFlagDropOff) ||
             (move_check.extended_flags & kExtendedFlagEdgeWalker)) &&
    ...
    if (move_check.flags & kMapObjectFlagMissile)
    {
        ...
        if (move_check.z > thing->z + thing->height_) return true;  // overhead
    ...
    if ((move_check.flags & kMapObjectFlagPickup) &&
        (thing->flags_ & kMapObjectFlagSpecial))
    {
        // can remove thing
...
static bool CheckRelativePosition(MapObject *thing, float x, float y)
{
    ...
    if (move_check.subsector->sector->floor_vertex_slope)
    {
        ...
        float    z_test =
            MathLinePlaneIntersection(
                line_a, line_b, move_check.subsector->sector->floor_z_verts[2],
                move_check.subsector->sector->floor_vs_normal)
                .Z;
        if (isfinite(z_test))
            move_check.floor_slope_z =
                z_test - move_check.subsector->sector->f_h;
    }

    if (move_check.subsector->sector->ceil_vertex_slope)
    {
        HMM_Vec3 line_a{{move_check.x, move_check.y, -40000}};
        HMM_Vec3 line_b{{move_check.x, move_check.y, 40000}};
        float    z_test =
            MathLinePlaneIntersection(
                line_a, line_b, move_check.subsector->sector->ceil_z_verts[2],
                move_check.subsector->sector->ceil_vs_normal)
                .Z;
        if (isfinite(z_test))
            move_check.ceiling_slope_z =
                move_check.subsector->sector->c_h - z_test;
    }
    ...
    ComputeThingGap(thing, move_check.subsector->sector, move_check.z,
                    &move_check.floor_z, &move_check.ceiling_z,
    ...
    // can go anywhere
    if (move_check.flags & kMapObjectFlagNoClip) return true;
...
bool TryMove(MapObject *thing, float x, float y)
{
    ...
    // solid wall or thing ?
    if (!CheckRelativePosition(thing, x, y)) return false;
    ...
    if (!(thing->flags_ & kMapObjectFlagNoClip))
    {
        ...
        if (!(thing->flags_ & kMapObjectFlagTeleport) &&
            (thing->z + thing->height_ > move_check.ceiling_z))
        {
        ...
        if (!fell_off_thing &&
            (thing->extended_flags_ & kExtendedFlagMonster) &&
        ...
        if (!fell_off_thing &&
            ...
            !((thing->flags_ & (kMapObjectFlagDropOff | kMapObjectFlagFloat)) ||
              (thing->extended_flags_ &
               (kExtendedFlagEdgeWalker | kExtendedFlagWaterWalker))) &&
            (move_check.floor_z - move_check.dropoff >
    ...
    if (!special_lines_hit.empty() &&
        !(thing->flags_ & (kMapObjectFlagTeleport | kMapObjectFlagNoClip)))
    {
        // Thing doesn't change, so we check the notriggerlines flag once..
        if (thing->player_ || (thing->extended_flags_ & kExtendedFlagMonster) ||
            !(thing->current_attack_ &&
              (thing->current_attack_->flags_ & kAttackFlagNoTriggerLines)))
        {
            for (auto iter = special_lines_hit.rbegin();
...
static bool ThingHeightClip(MapObject *thing)
{
    ...
    if (thing->ceiling_z_ - thing->floor_z_ < thing->height_) return false;
...
static bool PTR_SlideTraverse(PathIntercept *in, void *dataptr)
{
    ...
    if (!is_blocking)
    {
        ...
        for (int i = 0; i < ld->gap_num; i++)
        {
            ...
            if (slide_map_object->height_ > ld->gaps[i].c - ld->gaps[i].f)
                continue;

            // check slide mobj is not too high
            if (slide_map_object->z + slide_map_object->height_ > ld->gaps[i].c)
                continue;

            // check slide mobj can step over
            if (slide_map_object->z + slide_map_object->info_->step_size_ <
                ld->gaps[i].f)
                continue;
...
void P_SlideMove(MapObject *mo, float x, float y)
{
    ...
    for (int hitcount = 0; hitcount < 2; hitcount++)
    {
        ...
        PathTraverse(leadx, leady, leadx + dx, leady + dy, kPathAddLines,
                     PTR_SlideTraverse);
        PathTraverse(trailx, leady, trailx + dx, leady + dy, kPathAddLines,
                     PTR_SlideTraverse);
        PathTraverse(leadx, traily, leadx + dx, traily + dy, kPathAddLines,
                     PTR_SlideTraverse);
        ...
        if (best_slide_along > 1.0f) best_slide_along = 1.0f;

        if (best_slide_along <= 0.0f) return;
        ...
        if (TryMove(mo, mo->x + slide_move_x, mo->y + slide_move_y)) return;
    }

    // stairstep: last ditch attempt
    if (!TryMove(mo, mo->x, mo->y + dy)) TryMove(mo, mo->x + dx, mo->y);
}
...
static bool PTR_AimTraverse(PathIntercept *in, void *dataptr)
{
    ...
    if (in->line)
    {
        ...
        if (!AlmostEquals(ld->frontsector->f_h, ld->backsector->f_h))
        {
            ...
            if (slope > aim_check.bottom_slope) aim_check.bottom_slope = slope;
        }
        ...
        if (!AlmostEquals(ld->frontsector->c_h, ld->backsector->c_h))
        {
            ...
            if (slope < aim_check.top_slope) aim_check.top_slope = slope;
        }
    ...
    if (aim_check.source && !aim_check.forced &&
        (aim_check.source->side_ & mo->side_) != 0)
...
static bool PTR_AimTraverse2(PathIntercept *in, void *dataptr)
{
    ...
    if (aim_check.source &&
        (aim_check.source->side_ & mo->side_) == 0)  // not a friend
    {
    ...
    if (mo->flags_ & kMapObjectFlagCorpse) return true;  // don't aim at corpses
...
static inline bool ShootCheckGap(float sx, float sy, float z, float f_h,
                                 surface_t *floor, float c_h, surface_t *ceil,
                                 sector_t *sec_check, line_t *ld)
{
    ...
    if (AlmostEquals(shoot_check.slope, 0.0f) &&
        (!sec_check ||
         (!sec_check->floor_vertex_slope && !sec_check->ceil_vertex_slope)))
        return true;
    ...
    if (sec_check && sec_check->floor_vertex_slope)
    {
        if (sec_check->floor_vs_hilo.X > sec_check->f_h)
        {
            ...
            {
                if (AlmostEquals(ld->v1->X, v.X) &&
                    AlmostEquals(ld->v1->Y, v.Y))
                {
                    tri_v1.X = v.X;
                    tri_v1.Y = v.Y;
                    tri_v1.Z = v.Z;
                }
                else if (AlmostEquals(ld->v2->X, v.X) &&
                         AlmostEquals(ld->v2->Y, v.Y))
                {
                    tri_v2.X = v.X;
                    tri_v2.Y = v.Y;
                    tri_v2.Z = v.Z;
            ...
            if (AlmostEquals(tri_v1.Z, tri_v2.Z) &&
                AlmostEquals(
                    HMM_Clamp(HMM_MIN(sec_check->f_h, tri_v1.Z), z,
                              HMM_MAX(sec_check->f_h, tri_v1.Z)),
                    z))  // Hitting rectangular side; no fancier check needed
            {
                if (shoot_check.puff)
                {
                    sx -= trace.dx * 6.0f / shoot_check.range;
                    sy -= trace.dy * 6.0f / shoot_check.range;
            ...
            else
            {
                ...
                if (std::abs(tri_v1.X - tri_v2.X) >
                    std::abs(tri_v1.Y - tri_v2.Y))
                {
                    if (MathPointInTriangle(
                            {{tri_v1.X, tri_v1.Z}}, {{tri_v2.X, tri_v2.Z}},
                            {{(tri_v1.Z > tri_v2.Z ? tri_v1.X : tri_v2.X),
                              sec_check->f_h}},
                            {{sx, z}}))
                    {
                        if (shoot_check.puff)
                ...
                {
                    if (MathPointInTriangle(
                            {{tri_v1.Y, tri_v1.Z}}, {{tri_v2.Y, tri_v2.Z}},
                            {{(tri_v1.Z > tri_v2.Z ? tri_v1.Y : tri_v2.Y),
                              sec_check->f_h}},
                            {{sy, z}}))
                    {
                        if (shoot_check.puff)
    ...
    if (sec_check && sec_check->ceil_vertex_slope)
    {
        if (sec_check->ceil_vs_hilo.Y < sec_check->c_h)
        {
            ...
            if (AlmostEquals(tri_v1.Z, tri_v2.Z) &&
                AlmostEquals(
                    HMM_Clamp(HMM_MIN(sec_check->c_h, tri_v1.Z), z,
                              HMM_MAX(sec_check->c_h, tri_v1.Z)),
                    z))  // Hitting rectangular side; no fancier check needed
            {
                if (shoot_check.puff)
                {
    ...
    if (shoot_check.previous_z > f_h && z < f_h)
    { /* nothing */
    }
    // check if hit the ceiling
    else if (shoot_check.previous_z < c_h && z > c_h)
    {
        f_h   = c_h;
        floor = ceil;
    }
    else
    {
        if (sec_check && sec_check->floor_vertex_slope)
        {
            ...
            HMM_Vec3 shoota = MathLinePlaneIntersection(
                {{shoot_check.source->x, shoot_check.source->y,
                  shoot_check.start_z}},
                {{sx, sy, z}}, sec_check->floor_z_verts[2],
                sec_check->floor_vs_normal);
            sector_t *shoota_sec =
                R_PointInSubsector(shoota.X, shoota.Y)->sector;
            if (shoota_sec && shoota_sec == sec_check &&
                shoota.Z <= sec_check->floor_vs_hilo.X &&
                shoota.Z >= sec_check->floor_vs_hilo.Y)
            {
                ...
                if (PathTraverse(sx, sy, shoota.X, shoota.Y, kPathAddThings,
            ...
            else if (sec_check->ceil_vertex_slope)
            {
                ...
                shoota = MathLinePlaneIntersection(
                    {{shoot_check.source->x, shoot_check.source->y,
                      shoot_check.start_z}},
                    {{sx, sy, z}}, sec_check->ceil_z_verts[2],
                    sec_check->ceil_vs_normal);
                shoota_sec = R_PointInSubsector(shoota.X, shoota.Y)->sector;
                if (shoota_sec && shoota_sec == sec_check &&
                    shoota.Z <= sec_check->ceil_vs_hilo.X &&
    ...
    float along =
        (f_h - shoot_check.start_z) / (shoot_check.slope * shoot_check.range);

    float x = trace.x + trace.dx * along;
    float y = trace.y + trace.dy * along;

    z = (z < shoot_check.previous_z) ? f_h + 2 : f_h - 2;
    ...
    if (last_shoota_sec && (last_shoota_sec->floor_vertex_slope ||
                            last_shoota_sec->ceil_vertex_slope))
    {
        bool fs_good = true;
        bool cs_good = true;
        if (last_shoota_sec->floor_vertex_slope)
        {
            if (z <=
                MathLinePlaneIntersection({{x, y, -40000}}, {{x, y, 40000}},
                                          last_shoota_sec->floor_z_verts[2],
        ...
        {
            if (z >= MathLinePlaneIntersection({{x, y, -40000}},
                                               {{x, y, 40000}},
                                               last_shoota_sec->ceil_z_verts[2],
                                               last_shoota_sec->ceil_vs_normal)
                         .Z)
                cs_good = false;
        }
        if (fs_good && cs_good) return true;
    }
...
bool ReplaceMidTexFromPart(line_t *TheLine, ScrollingPart parts)
{
    ...
    side_t *side = (IsFront) ? TheLine->side[0] : TheLine->side[1];
    ...
    if (parts & (kScrollingPartLeftUpper)) { image = side->top.image; }
    if (parts & (kScrollingPartRightUpper)) { image = side->top.image; }
    if (parts & (kScrollingPartLeftLower)) { image = side->bottom.image; }
    if (parts & (kScrollingPartRightLower)) { image = side->bottom.image; }
    ...
    if (TheLine->side[1]) TheLine->side[1]->middle.image = image;
...
void P_UnblockLineEffectDebris(line_t *TheLine, const LineType *special)
{
    ...
    if (TheLine->side[0] && TheLine->side[1]) TwoSided = true;
...
static bool ShootTraverseCallback(PathIntercept *in, void *dataptr)
{
    ...
    if (in->line)
    {
        ...
        float y     = trace.y + trace.dy * along;
        float z =
            shoot_check.start_z + along * shoot_check.slope * shoot_check.range;
        ...
        if (side)
        {
            ...
            {
                if (!ShootCheckGap(x, y, z, floor_h, floor_s, ef->bottom_h,
                                   ef->bottom, sec_check, ld))
                    return false;
            ...
            if (!ShootCheckGap(x, y, z, floor_h, floor_s, side->sector->c_h,
                               &side->sector->ceil, sec_check, ld))
            {
                return false;
            }
        }
        ...
        if (ld->special &&
            (!shoot_check.source || !shoot_check.source->current_attack_ ||
             !(shoot_check.source->current_attack_->flags_ &
               kAttackFlagNoTriggerLines)))
        {
        ...
        if (ld->frontsector && ld->backsector)
        {
            ...
            if (IS_SKY(ld->frontsector->floor) && IS_SKY(ld->backsector->floor))
            {
                ...
                if (HMM_MIN(f1, f2) <= z && z <= HMM_MAX(f1, f2)) return false;
            }
        }
        ...
        if (last_shoota_sec &&
            ((ld->frontsector && (ld->frontsector->floor_vertex_slope ||
                                  ld->frontsector->ceil_vertex_slope)) ||
             (ld->backsector && (ld->backsector->floor_vertex_slope ||
                                 ld->backsector->ceil_vertex_slope))))
        {
            bool fs_good = true;
            bool cs_good = true;
            if (last_shoota_sec->floor_vertex_slope)
            ...
            {
                if (z <= last_shoota_sec->f_h) fs_good = false;
            }
            if (last_shoota_sec->ceil_vertex_slope)
            {
                if (z >=
            ...
            {
                if (z >= last_shoota_sec->c_h) cs_good = false;
            }
            if (fs_good && cs_good) return true;
        }

        // position puff off the wall
        x -= trace.dx * 6.0f / shoot_check.range;
        y -= trace.dy * 6.0f / shoot_check.range;
    ...
    if (!(mo->flags_ & kMapObjectFlagShootable) &&
        !(mo->extended_flags_ & kExtendedFlagBlockShots))
        return true;
    ...
    // shot over the thing ?
    if (thingtopslope < shoot_check.slope) return true;

    float thingbottomslope = (mo->z - shoot_check.start_z) / dist;

    // shot under the thing ?
    if (thingbottomslope > shoot_check.slope) return true;
    ...
    bool use_blood = (mo->flags_ & kMapObjectFlagShootable) &&
                     !(mo->flags_ & kMapObjectFlagNoBlood) &&
                     (gore_level.d_ < 2);
                           ...
                           float *slope)
{
    ...
    PathTraverse(t1->x, t1->y, x2, y2, kPathAddLines | kPathAddThings,
                 PTR_AimTraverse);

    if (slope) (*slope) = aim_check.slope;
                               ...
                               float distance)
{
    ...
    if (source->info_)
        aim_check.start_z =
            source->z + source->height_ * source->info_->shotheight_;
    else
    ...
    PathTraverse(source->x, source->y, x2, y2, kPathAddLines | kPathAddThings,
                 PTR_AimTraverse2);

    if (!aim_check.target) return nullptr;
                              ...
                              bool force_aim)
{
    ...
    if (!target)
    {
        ...
        if (level_time_elapsed & 1) diff = 0 - diff;

        MapObject *T2 =
            DoMapTargetAutoAim(source, angle + diff, distance, force_aim);
        if (T2) return T2;
...
static bool PTR_UseTraverse(PathIntercept *in, void *dataptr)
{
    ...
    if (in->thing)
    {
        ...
        // not a usable thing ?
        if (!(mo->extended_flags_ & kExtendedFlagUsable) ||
    ...
    if (!ld->special || ld->special->type_ == kLineTriggerShootable ||
        ld->special->type_ == kLineTriggerWalkable)
    {
...
static bool RadiusAttackCallback(MapObject *thing, void *data)
{
    ...
    if ((thing->hyper_flags_ & kHyperFlagFriendlyFireImmune) &&
        radius_attack_check.source &&
    ...
    if (thing->info_->splash_group_ >= 0 &&
        ...
        (thing->info_->splash_group_ ==
         radius_attack_check.source->info_->splash_group_))
    {
        return true;
    }
    ...
    {
        if (!radius_attack_check.source) return true;
        // MBF21 FORCERADIUSDMG flag
        if (!(radius_attack_check.source->mbf21_flags_ &
              kMBF21FlagForceRadiusDamage))
            return true;
    }
    ...
    dz = (float)fabs(MapObjectMidZ(thing) -
                     MapObjectMidZ(radius_attack_check.spot));
    ...
    if (P_CheckSight(radius_attack_check.spot, thing))
    {
        if (radius_attack_check.thrust)
            ThrustMapObject(thing, radius_attack_check.spot,
                            radius_attack_check.damage * dist);
        else
            DamageMapObject(thing, radius_attack_check.spot,
                            radius_attack_check.source,
                            radius_attack_check.damage * dist,
                            radius_attack_check.damage_type);
    }
    return true;
}
                  ...
                  float damage, const DamageClass *damtype, bool thrust_only)
{
    ...
    BlockmapThingIterator(spot->x - r, spot->y - r, spot->x + r, spot->y + r,
                          RadiusAttackCallback);
}
...
static bool ChangeSectorCallback(MapObject *thing, bool widening)
{
    ...
    if (!(thing->flags_ & kMapObjectFlagShootable) ||
        (thing->flags_ & kMapObjectFlagNoClip))
        return true;
...
//
static void ChangeSectorHeights(sector_t *sec, float f_h, float c_h, float f_dh,
                                float c_dh)
{
...
bool CheckSolidSectorMove(sector_t *sec, bool is_ceiling, float dh)
{
    ...
    if (AlmostEquals(dh, 0.0f)) return true;
    ...
    {
        if (sec->ceil_move) sec->ceil_move->destination_height = sec->f_h - dh;
    }

    // don't allow a dummy sector to go FUBAR
    if (sec->control_floors)
    {
        if (is_ceiling && (sec->c_h + dh < sec->f_h)) return false;

        if (!is_ceiling && (sec->f_h + dh > sec->c_h)) return false;
    }
    ...
    for (ef = sec->control_floors; ef; ef = ef->ctrl_next)
    {
        ...
        if (ef->ef_info->type_ & kExtraFloorTypeLiquid) continue;
        ...
        if (!is_ceiling && !(ef->ef_info->type_ & kExtraFloorTypeThick))
        {
            ...
            if (dh > 0 &&
                new_h > (ef->higher ? ef->higher->bottom_h : ef->sector->c_h))
            {
                return false;
            }

            if (dh < 0 &&
                new_h < (ef->lower ? ef->lower->top_h : ef->sector->f_h))
            {
                return false;
            }
            continue;
        ...
        if (is_ceiling && (ef->ef_info->type_ & kExtraFloorTypeThick))
        {
            ...
            if (dh < 0 && new_h < ef->bottom_h) return false;
        ...
        if (!is_ceiling && (ef->ef_info->type_ & kExtraFloorTypeThick))
        {
            ...
            if (dh > 0 && new_h > ef->top_h) return false;
...
//
bool SolidSectorMove(sector_t *sec, bool is_ceiling, float dh, int crush,
                     bool nocarething)
{
    extrafloor_t *ef;

    if (AlmostEquals(dh, 0.0f)) return false;
    ...
    for (ef = sec->control_floors; ef; ef = ef->ctrl_next)
    {
        ...
        else { ef->top_h = ef->bottom_h = sec->f_h; }
    ...
    if (!nocarething)
    {
        for (ef = sec->control_floors; ef; ef = ef->ctrl_next)
        {
            ...
            if (!is_ceiling && !(ef->ef_info->type_ & kExtraFloorTypeThick))
            {
                ...
                {
                    float h =
                        ef->higher ? ef->higher->bottom_h : ef->sector->c_h;
...
static bool CorpseCheckCallback(MapObject *thing, void *data)
{
    ...
    if (!(thing->flags_ & kMapObjectFlagCorpse)) return true;  // not a corpse

    if (thing->tics_ != -1) return true;  // not lying still yet
    ...
    if (fabs(thing->x - raiser_try_x) > maxdist ||
        fabs(thing->y - raiser_try_y) > maxdist)
    ...
    if (thing->player_ &&
        (raiser_try_object->info_->side_ & thing->info_->side_) == 0)
        return true;
...
MapObject *FindCorpseForResurrection(MapObject *thing)
{
    if (thing->move_direction_ != kDirectionNone)
    {
        ...
        raiser_try_x =
            thing->x + thing->speed_ * xspeed[thing->move_direction_];
        raiser_try_y =
            thing->y + thing->speed_ * yspeed[thing->move_direction_];

        if (!BlockmapThingIterator(
                raiser_try_x - kRaiseRadius, raiser_try_y - kRaiseRadius,
                raiser_try_x + kRaiseRadius, raiser_try_y + kRaiseRadius,
                CorpseCheckCallback))
        {
...
static bool CheckBlockingLineCallback(line_t *line, void *data)
{
    ...
    for (int i = 0; i < line->gap_num; i++)
    {
        ...
        if (line->gaps[i].f <= mb2 && mt2 <= line->gaps[i].c) return true;
    }
    ...
    if (slope_sec &&
        (slope_sec->floor_vertex_slope || slope_sec->ceil_vertex_slope))
    {
        bool fs_good = true;
        bool cs_good = true;
        if (slope_sec->floor_vertex_slope)
        {
            if (mb2 <= MathLinePlaneIntersection({{mx2, my2, -40000}},
                                                 {{mx2, my2, 40000}},
                                                 slope_sec->floor_z_verts[2],
        ...
        {
            if (mt2 >= MathLinePlaneIntersection({{mx2, my2, -40000}},
                                                 {{mx2, my2, 40000}},
                                                 slope_sec->ceil_z_verts[2],
...
bool MapCheckBlockingLine(MapObject *thing, MapObject *spawnthing)
{
    ...
    crosser = (spawnthing->extended_flags_ & kExtendedFlagCrossBlockingLines)
                  ? true
                  : false;
    ...
    if (!BlockmapLineIterator(HMM_MIN(mx1, mx2), HMM_MIN(my1, my2),
                              HMM_MAX(mx1, mx2), HMM_MAX(my1, my2),
                              CheckBlockingLineCallback))
    {
        return true;
    }
...
{
    for (line_t *s : special_lines_hit)
    {
        delete s;
        s = nullptr;
    }
