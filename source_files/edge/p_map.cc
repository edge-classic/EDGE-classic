...
static bool PIT_CheckRelLine(line_t *ld, void *data)
{
    ...
    if (ld->frontsector->floor_vertex_slope || ld->backsector->floor_vertex_slope)
    {
            ...
            R_PointInSubsector(tm_I.mover->x, tm_I.mover->y)->sector != ld->frontsector)
        {
            ...
            if (std::isfinite(ix) && std::isfinite(iy))
            {
                float icz = M_LinePlaneIntersection({{ix, iy, -40000}}, {{ix, iy, 40000}}, ld->frontsector->ceil_z_verts[2],
                 ...
                 R_PointInSubsector(tm_I.mover->x, tm_I.mover->y)->sector != ld->backsector)
        {
            ...
            if (std::isfinite(ix) && std::isfinite(iy))
            {
                float icz = M_LinePlaneIntersection({{ix, iy, -40000}}, {{ix, iy, 40000}}, ld->backsector->ceil_z_verts[2],
                 ...
                 R_PointInSubsector(tm_I.mover->x, tm_I.mover->y)->sector == ld->frontsector)
        {
            ...
            else
            {
                ...
                {
                    float icz =
                        M_LinePlaneIntersection({{ix, iy, -40000}}, {{ix, iy, 40000}}, ld->backsector->ceil_z_verts[2],
                                                ld->backsector->ceil_vs_normal).Z;
                    if (std::isfinite(icz) && icz <= iz + tm_I.mover->height)
                    {
                        blockline = ld;
                 ...
                 R_PointInSubsector(tm_I.mover->x, tm_I.mover->y)->sector == ld->backsector)
        {
            ...
            else
            {
                ...
                {
                    float icz =
                        M_LinePlaneIntersection({{ix, iy, -40000}}, {{ix, iy, 40000}}, ld->frontsector->ceil_z_verts[2],
                                                ld->frontsector->ceil_vs_normal).Z;
                    if (std::isfinite(icz) && icz <= iz + tm_I.mover->height)
                    {
                        blockline = ld;
...
static bool P_CheckRelPosition(mobj_t *thing, float x, float y)
{
    ...
    if (tm_I.sub->sector->floor_vertex_slope)
    {
        ...
        float  z_test = M_LinePlaneIntersection(line_a, line_b, tm_I.sub->sector->floor_z_verts[2],
    ...
    if (tm_I.sub->sector->ceil_vertex_slope)
    {
        HMM_Vec3 line_a{{tm_I.x, tm_I.y, -40000}};
        HMM_Vec3 line_b{{tm_I.x, tm_I.y, 40000}};
        float  z_test = M_LinePlaneIntersection(line_a, line_b, tm_I.sub->sector->ceil_z_verts[2],
...
static inline bool ShootCheckGap(float sx, float sy, float z, float f_h, surface_t *floor, float c_h, surface_t *ceil,
                                 sector_t *sec_check, line_t *ld)
{
    /* Returns true if successfully passed gap */

    // perfectly horizontal shots cannot hit planes
    if (AlmostEquals(shoot_I.slope, 0.0f) &&
        (!sec_check || (!sec_check->floor_vertex_slope && !sec_check->ceil_vertex_slope)))
        return true;
    ...
    else
    {
        if (sec_check && sec_check->floor_vertex_slope)
        {
            ...
            HMM_Vec3 shoota = M_LinePlaneIntersection(
                {{shoot_I.source->x, shoot_I.source->y, shoot_I.start_z}}, {{sx, sy, z}}, sec_check->floor_z_verts[2], sec_check->floor_vs_normal);
        ...
        else if (sec_check && sec_check->ceil_vertex_slope)
        {
            ...
            HMM_Vec3    shoota     = M_LinePlaneIntersection({{shoot_I.source->x, shoot_I.source->y, shoot_I.start_z}},
                                                           {{sx, sy, z}}, sec_check->ceil_z_verts[2], sec_check->ceil_vs_normal);
            sector_t *shoota_sec = R_PointInSubsector(shoota.X, shoota.Y)->sector;
            if (shoota_sec && shoota_sec == sec_check && shoota.Z <= sec_check->ceil_vs_hilo.X &&
                shoota.Z >= sec_check->ceil_vs_hilo.Y)
            {
    ...
    if (last_shoota_sec && (last_shoota_sec->floor_vertex_slope || last_shoota_sec->ceil_vertex_slope))
    {
        ...
        {
            if (z <= M_LinePlaneIntersection({{x, y, -40000}}, {{x, y, 40000}}, last_shoota_sec->floor_z_verts[2],
        ...
        {
            if (z >= M_LinePlaneIntersection({{x, y, -40000}}, {{x, y, 40000}}, last_shoota_sec->ceil_z_verts[2],
...
static bool PTR_ShootTraverse(intercept_t *in, void *dataptr)
{
    ...
    if (in->line)
    {
        ...
        if (side)
        {
            ...
            if (!ShootCheckGap(x, y, z, floor_h, floor_s, side->sector->c_h, &side->sector->ceil, sec_check, ld))
            {
                return false;
            }
        }
        ...
        if ((ld->flags & MLF_TwoSided) && ld->gap_num > 0 && !(ld->flags & MLF_ShootBlock))
        {
        ...
        if (last_shoota_sec &&
            ((ld->frontsector && (ld->frontsector->floor_vertex_slope || ld->frontsector->ceil_vertex_slope)) ||
...
static bool PIT_RadiusAttack(mobj_t *thing, void *data)
{
    ...
    if (P_CheckSight(bomb_I.spot, thing))
    {
        if (bomb_I.thrust)
            ...
            P_DamageMobj(thing, bomb_I.spot, bomb_I.source, bomb_I.damage * dist, bomb_I.damtype);
    }
    return true;
}
...
bool P_CheckSolidSectorMove(sector_t *sec, bool is_ceiling, float dh)
{
    ...
    if (is_ceiling && dh < 0 && sec->top_ef && (sec->c_h - dh < sec->top_ef->top_h))
    {
        return false;
    }

    if (!is_ceiling && dh > 0 && sec->bottom_ef && (sec->f_h + dh > sec->bottom_ef->bottom_h))
    {
        return false;
    }
    ...
    for (ef = sec->control_floors; ef; ef = ef->ctrl_next)
    {
        ...
        if (!is_ceiling && !(ef->ef_info->type_ & kExtraFloorTypeThick))
        {
            ...
            if (dh > 0 && new_h > (ef->higher ? ef->higher->bottom_h : ef->sector->c_h))
            {
                return false;
            }

            if (dh < 0 && new_h < (ef->lower ? ef->lower->top_h : ef->sector->f_h))
            {
                return false;
            }
            continue;
...
//
bool P_SolidSectorMove(sector_t *sec, bool is_ceiling, float dh, int crush, bool nocarething)
{
    extrafloor_t *ef;
...
static bool PIT_CheckBlockingLine(line_t *line, void *data)
{
    ...
    if ((crosser && (line->flags & MLF_ShootBlock)) ||
        (!crosser &&
         (line->flags & (MLF_Blocking | MLF_BlockMonsters)))) // How to handle MLF_BlockGrounded and MLF_BlockPlayer?
    {
        blockline = line;
        return false;
    }
    ...
    if (slope_sec && (slope_sec->floor_vertex_slope || slope_sec->ceil_vertex_slope))
    {
        ...
        {
            if (mb2 <= M_LinePlaneIntersection({{mx2, my2, -40000}}, {{mx2, my2, 40000}}, slope_sec->floor_z_verts[2],
        ...
        {
            if (mt2 >= M_LinePlaneIntersection({{mx2, my2, -40000}}, {{mx2, my2, 40000}}, slope_sec->ceil_z_verts[2],
