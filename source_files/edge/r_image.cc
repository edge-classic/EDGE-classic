...
{
    epi::StringHash name_hash = epi::StringHash::Create(tex_name);
...
static const Image *BackupFlat(const char *flat_name, int flags)
{
    ...
    if (!(flags & kImageLookupExact))
    {
        rim = ImageContainerLookupInternal(real_textures, epi::StringHash::Create(flat_name));
        if (rim)
            return rim;
    }
...
static const Image *BackupGraphic(const char *gfx_name, int flags)
{
    ...
    epi::StringHash name_hash = epi::StringHash::Create(gfx_name);
