...
static const char *rad_level_names[3] = {"outer area", "map area", "trigger area"};
...
void RAD_Error(const char *err, ...)
{
    ...
    if (buffer[2047] != 0)
        FatalError("Buffer overflow in RAD_Error.\n");
...
//
static void AddStateToScript(rad_script_t *R, int tics, void (*action)(struct rad_trigger_s *R, void *param),
                             void *param)
{
...
static void RAD_ParseWhenPlayerNum(param_set_t &pars)
{
    ...
    {
        RAD_Error("%s: Illegal range: %d..%d\n", pars[0], this_rad->min_players, this_rad->max_players);
    }
}
        ...
        if (cur->level >= 0)
        {
            if (cur->level != rad_cur_level)
            {
                RAD_Error("RTS command '%s' used in wrong place "
                          ...
                          pars[0], rad_level_names[rad_cur_level], rad_level_names[cur->level]);
