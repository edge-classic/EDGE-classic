...
class IMFPlayer : public AbstractMusicPlayer
{
  ...
  public:
    bool OpenMemory(uint8_t *data, int length)
    {
        ...
        if (ma_sound_init_from_data_source(&music_engine, &imf_decoder_, MA_SOUND_FLAG_STREAM|MA_SOUND_FLAG_UNKNOWN_LENGTH|MA_SOUND_FLAG_NO_SPATIALIZATION, NULL, &imf_stream_) != MA_SUCCESS)
        {
