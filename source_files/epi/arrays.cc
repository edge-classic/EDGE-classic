...
// array_c constructor
//
array_c::array_c(const int objsize) : array(NULL), array_end(NULL), array_entries(0), array_max_entries(0)
{
    // Size to the nearest block
    array_block_objsize = (objsize + (sizeof(array_block_t) - 1)) / sizeof(array_block_t);
...
int array_c::InsertObject(void *obj, int pos)
{
    ...
    {
        memmove(FetchObjectDirect(pos + 1), FetchObjectDirect(pos),
                sizeof(array_block_t) * array_block_objsize * (array_entries - pos));
    }
...
void array_c::RemoveObject(int pos)
{
    ...
    if (pos < array_entries)
    {
        memmove(FetchObjectDirect(pos), FetchObjectDirect(pos + 1),
                sizeof(array_block_t) * array_block_objsize * (array_entries - pos));
    }
...
{
    array_iterator_c tail_iterator(this, array_entries > 1 ? array_entries - 1 : 0);
