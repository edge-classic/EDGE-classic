...
bool IsPathAbsolute(std::string_view path)
{
    ...
    if (path.size() > 2 && path[1] == ':' &&
        (path[2] == '\\' || path[2] == '/') && IsAlphaASCII(path[0]))
    {
        return true;
    }
...
static std::string CurrentDirectoryGet()
{
    ...
    if (dir) directory = epi::WStringToUTF8(dir);
...
bool ReadDirectory(std::vector<DirectoryEntry> &fsd, std::string &dir,
                   const char *mask)
{
    if (dir.empty() || !FileExists(dir) || !mask) return false;
    ...
    if (!CurrentDirectorySet(dir)) return false;
    ...
    do {
        ...
        else
        {
            ...
            new_entry.name = dir;
            new_entry.is_dir =
                (fdataw.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) ? true
                                                                     : false;
...
{
    if (dir.empty() || !FileExists(dir)) return false;
    ...
    do {
        ...
        else if (fdataw.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY)
        {
            ...
            if (!WalkDirectory(fsd, subdir)) return false;
        }
        else
        {
            epi::DirectoryEntry new_entry;
                   ...
                   const char *mask)
{
    ...
    std::string mask_ext =
        epi::GetExtension(mask);  // Allows us to retain "*.*" style syntax
    ...
    for (;;)
    {
        ...
        if (mask_ext != ".*" && epi::StringCaseCompareASCII(
                                    mask_ext, epi::GetExtension(filename)) != 0)
            continue;

        struct stat finfo;

        if (stat(filename.c_str(), &finfo) != 0) continue;
...
std::string GetStem(std::string_view path)
{
    ...
    for (int p = path.size() - 2; p >= 0; p--)
    {
        ...
        if (ch == '.')
        {
            ...
            if (p == 0 || IsDirectorySeparator(path[p - 1])) break;
...
void ReplaceExtension(std::string &path, std::string_view ext)
{
    ...
    {
        while (path.size() > extpos) { path.pop_back(); }
        path.append(ext);
    }
}
...
bool FileCopy(std::string_view src, std::string_view dest)
{
    ...
    if (epi::FileExists(dest))
    {
        ...
        if (!epi::FileDelete(dest)) return false;
    }
...
void SyncFilesystem(bool populate)
{
    EM_ASM_(
        {
            if (Module.edgePreSyncFS) { Module.edgePreSyncFS(); }
            FS.syncfs(
                $0, function(err) {
                    if (Module.edgePostSyncFS) { Module.edgePostSyncFS(); }
                });
        },
        populate);
}
#else
void SyncFilesystem(bool populate) { (void)populate; }
#endif
