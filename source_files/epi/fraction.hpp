...
template <typename inttype = int> class fraction
{
    ...
    {
        cerr << nom() << '/' << denom() << ' ' << op << ' ' << b.nom() << '/' << denom() << ":\n";
    }
#endif
  public:
    void set(inttype n, inttype d)
    {
        num1 = n;
        num2 = d;
        Optim();
    ...
    template <typename floattype> explicit fraction(const floattype value)
    {
        operator=(value);
    }
    ...
    inline long double valuel() const
    {
        return nom() / (long double)denom();
    }
    self &operator+=(const inttype &value)
    {
        num1 += value * denom();
        Optim();
        return *this;
...
#define fraction_blah_func(op1, op2)                                                                                   \
    self operator op1(const self &b) const                                                                             \
    {                                                                                                                  \
        self    tmp(*this);                                                                                            \
        tmp op2 b;                                                                                                     \
        return tmp;                                                                                                    \
    ...
    fraction_blah_func(+, +=) fraction_blah_func(-, -=) fraction_blah_func(/, /=) fraction_blah_func(*, *=)

#undef fraction_blah_func
#define fraction_blah_func(op)                                                                                         \
    bool operator op(const self &b) const                                                                              \
    {                                                                                                                  \
        return value() op b.value();                                                                                   \
    }                                                                                                                  \
    bool operator op(inttype b) const                                                                                  \
    {                                                                                                                  \
        return value() op b;                                                                                           \
    }
...
#undef fraction_blah_func
            ...
            const inttype &nom() const
    {
        return num1;
    }
    const inttype &denom() const
    {
        return num2;
    }
    inline bool operator==(inttype b) const
    {
        return denom() == 1 && nom() == b;
    }
    inline bool operator!=(inttype b) const
    {
        return denom() != 1 || nom() != b;
    ...
    {
        return denom() == b.denom() && nom() == b.nom();
    }
    inline bool operator!=(const self &b) const
    {
        return denom() != b.denom() || nom() != b.nom();
    ...
    self &operator=(double orig)
    {
        return *this = (long double)orig;
    }
...
template <typename inttype> void fraction<inttype>::Optim()
{
    /* Euclidean algorithm */
...
template <typename inttype> inline const fraction<inttype> abs(const fraction<inttype> &f)
{
    return fraction<inttype>(abs(f.nom()), abs(f.denom()));
}

#define fraction_blah_func(op)                                                                                         \
    template <typename inttype> fraction<inttype> operator op(const inttype bla, const fraction<inttype> &b)           \
    {                                                                                                                  \
...
#define fraction_blah_func(op1, op2)                                                                                   \
    template <typename inttype> fraction<inttype> &fraction<inttype>::operator op2(const fraction<inttype> &b)         \
    {                                                                                                                  \
    ...
    fraction_blah_func(+, +=) fraction_blah_func(-, -=)
#undef fraction_blah_func
