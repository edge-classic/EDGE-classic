...
int image_data_c::TrueWidth()
{
	if (bpp <= 3)
		return width; // Either no transparency or a palettised image not converted to RGBA for checking
	u8_t *src = pixels;
	int last_last = 0;
	int first_first = width;
	for (int i = 0; i < height; i++)
	{
		...
		if (found_first && first >= 0 && first < first_first)
			first_first = first;
		if (found_last && last <= width && last > last_last)
			last_last = last;
	}
	return last_last - first_first;
}

int image_data_c::TrueHeight()
{
	if (bpp <= 3)
		return height; // Either no transparency or a palettised image not converted to RGBA for checking
	u8_t *src = pixels;
	int last_last = 0;
	int first_first = height;
	for (int i = 0; i < width; i++)
	{
		...
		if (found_first && first >= 0 && first < first_first)
			first_first = first;
		if (found_last && last <= height && last > last_last)
			last_last = last;
	}
	return last_last - first_first;
}
