...
void Test_Angle_Conv()
{
    ...
    for (int i = -355; i < 360; i += 37)
    {
        ...
        printf("%+4d -> D=% 8.3f F=% 8.3f R=% 8.3f  |  % 9.6f -> rad=% 9.6f\n", i, D.Degrees(), F.Degrees(),
               R.Degrees(), rad, F.Radians());
    }
...
void Test_Angle_Const()
{
    ...
    printf("%3d = %10.6f  rad = %12.10f\n", 15, angle_c::Ang15().Degrees(), angle_c::Ang15().Radians());

    printf("%3d = %10.6f  rad = %12.10f\n", 30, angle_c::Ang30().Degrees(), angle_c::Ang30().Radians());

    printf("%3d = %10.6f  rad = %12.10f\n", 45, angle_c::Ang45().Degrees(), angle_c::Ang45().Radians());

    printf("%3d = %10.6f  rad = %12.10f\n", 60, angle_c::Ang60().Degrees(), angle_c::Ang60().Radians());

    printf("%3d = %10.6f  rad = %12.10f\n", 90, angle_c::Ang90().Degrees(), angle_c::Ang90().Radians());

    printf("%3d = %10.6f  rad = %12.10f\n", 145, angle_c::Ang145().Degrees(), angle_c::Ang145().Radians());

    printf("%3d = %10.6f  rad = %12.10f\n", 180, angle_c::Ang180().Degrees(), angle_c::Ang180().Radians());

    printf("%3d = %10.6f  rad = %12.10f\n", 225, angle_c::Ang225().Degrees(), angle_c::Ang225().Radians());

    printf("%3d = %10.6f  rad = %12.10f\n", 270, angle_c::Ang270().Degrees(), angle_c::Ang270().Radians());

    printf("%3d = %10.6f  rad = %12.10f\n", 315, angle_c::Ang315().Degrees(), angle_c::Ang315().Radians());

    printf("%3d = %10.6f  rad = %12.10f\n", 360, angle_c::Ang360().Degrees(), angle_c::Ang360().Radians());
...
void Test_Angle_Misc()
{
    ...
    for (int m = 0; m <= 360; m += 20)
    {
        ...
        printf("%3.0f -> Neg=%3.0f  Abs=%3.0f  Doub=%3.0f  Half=%3.0f  +90=%3.0f  +180=%3.0f\n", X.Degrees(),
               Neg.Degrees(), Abs.Degrees(), Doub.Degrees(), Half.Degrees(), J.Degrees(), K.Degrees());
    }
    ...
    for (int m = 9; m <= 360; m += 41)
        for (int n = 12; n <= 360; n += 60)
        {
            ...
            printf("%5.1f __ %5.1f:  <%c  <=%c  >%c  >=%c  ==%c  !=%c  |  <90%c  >180%c\n", A.Degrees(), B.Degrees(),
                   ...
                   (A < B) ? 'Y' : 'n', (A <= B) ? 'Y' : 'n', (A > B) ? 'Y' : 'n', (A >= B) ? 'Y' : 'n',
                   (A == B) ? 'Y' : 'n', (A != B) ? 'Y' : 'n',

                   (A.Less90()) ? 'Y' : 'n', (A.More180()) ? 'Y' : 'n');
        }
