...
class bbox3_c
{
  ...
  public:
    ...
    bbox3_c(const bbox2_c &rhs, float z1, float z2) : lo(rhs.lo, z1), hi(rhs.hi, z2)
    {
    }
...
inline bbox2_c::bbox2_c(const vec2_c &p1, const vec2_c &p2, const vec2_c &p3) : lo(p1), hi(p1)
{
    Insert(p2);
    Insert(p3);
}
...
{
    return (lo.x <= point.x && point.x <= hi.x && lo.y <= point.y && point.y <= hi.y);
}

inline bool bbox2_c::DoesIntersect(const bbox2_c &other) const
{
    return (other.hi.x > lo.x && other.lo.x < hi.x && other.hi.y > lo.y && other.lo.y < hi.y);
}
...
inline bbox3_c::bbox3_c(const vec3_c &p1, const vec3_c &p2, const vec3_c &p3) : lo(p1), hi(p1)
{
    Insert(p2);
    Insert(p3);
}
...
{
    return (lo.x <= point.x && point.x <= hi.x && lo.y <= point.y && point.y <= hi.y && lo.z <= point.z &&
            point.z <= hi.z);
}

inline bool bbox3_c::DoesIntersect(const bbox3_c &other) const
{
    return (other.hi.x > lo.x && other.lo.x < hi.x && other.hi.y > lo.y && other.lo.y < hi.y && other.hi.z > lo.z &&
            other.lo.z < hi.z);
}
