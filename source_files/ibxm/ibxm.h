...
// Adapted to single-file header use - Dasho
...
struct ibxm_module {
	...
	struct ibxm_pattern *patterns;
	struct ibxm_instrument *instruments;
};
...
struct ibxm_module* ibxm_module_load( struct ibxm_data *data, char *message );
/* Deallocate the specified module. */
void ibxm_dispose_module( struct ibxm_module *module );
/* Allocate and initialize a replay with the specified module and sampling rate. */
struct ibxm_replay* ibxm_new_replay( struct ibxm_module *module, int sample_rate, int interpolation );
/* Deallocate the specified replay. */
void ibxm_dispose_replay( struct ibxm_replay *replay );
/* Returns the song duration in samples at the current sampling rate. */
int ibxm_replay_calculate_duration( struct ibxm_replay *replay );
/* Seek to approximately the specified sample position.
   The actual sample position reached is returned. */
int ibxm_replay_seek( struct ibxm_replay *replay, int sample_pos );
/* Set the pattern in the sequence to play. The tempo is reset to the default. */
void ibxm_replay_set_sequence_pos( struct ibxm_replay *replay, int pos );
...
int ibxm_replay_get_audio( struct ibxm_replay *replay, int *mix_buf, int mute );
/* Returns the currently playing pattern in the sequence.*/
int ibxm_replay_get_sequence_pos( struct ibxm_replay *replay );
/* Returns the currently playing row in the pattern. */
int ibxm_replay_get_row( struct ibxm_replay *replay );
/* Returns the length of the output buffer required by replay_get_audio(). */
int ibxm_calculate_mix_buf_len( int sample_rate );

int ibxm_data_u16be( struct ibxm_data *data, int offset );
int ibxm_data_u16le( struct ibxm_data *data, int offset );
unsigned int ibxm_data_u32le( struct ibxm_data *data, int offset );
...
struct channel {
	struct ibxm_replay *replay;
	struct ibxm_instrument *instrument;
	struct ibxm_sample *sample;
	struct note note;
...
static char* data_ascii( struct ibxm_data *data, int offset, int length, char *dest ) {
	int idx, chr;
...
static int data_s8( struct ibxm_data *data, int offset ) {
	int value = 0;
	if( offset < data->length ) {
...
static int data_u8( struct ibxm_data *data, int offset ) {
	int value = 0;
	if( offset < data->length ) {
...
static void data_sam_s8( struct ibxm_data *data, int offset, int count, short *dest ) {
...
static void data_sam_s16le( struct ibxm_data *data, int offset, int count, short *dest ) {
	int idx, amp, length = data->length;
	char *buffer = data->buffer;
	if( offset > length ) {
		offset = length;
...
static int envelope_next_tick( struct ibxm_envelope *envelope, int tick, int key_on ) {
	tick++;
...
static int envelope_calculate_ampl( struct ibxm_envelope *envelope, int tick ) {
	int idx, point, dt, da;
...
static void sample_ping_pong( struct ibxm_sample *sample ) {
	int idx;
...
static struct ibxm_module* module_load_xm( struct ibxm_data *data, char *message ) {
	...
	struct ibxm_instrument *instrument;
	struct ibxm_sample *sample;
	struct ibxm_module *module = (ibxm_module *)calloc( 1, sizeof( struct ibxm_module ) );
	if( module ) {
		if( ibxm_data_u16le( data, 58 ) != 0x0104 ) {
			strcpy( message, "XM format version must be 0x0104!" );
			ibxm_dispose_module( module );
			return NULL;
		}
		data_ascii( data, 17, 20, module->name );
		delta_env = !memcmp( data_ascii( data, 38, 15, ascii ), "DigiBooster Pro", 15 );
		offset = 60 + ibxm_data_u32le( data, 60 );
		module->sequence_len = ibxm_data_u16le( data, 64 );
		module->restart_pos = ibxm_data_u16le( data, 66 );
		module->num_channels = ibxm_data_u16le( data, 68 );
		module->num_patterns = ibxm_data_u16le( data, 70 );
		module->num_instruments = ibxm_data_u16le( data, 72 );
		module->linear_periods = ibxm_data_u16le( data, 74 ) & 0x1;
		module->default_gvol = 64;
		module->default_speed = ibxm_data_u16le( data, 76 );
		module->default_tempo = ibxm_data_u16le( data, 78 );
		module->c2_rate = 8363;
		module->gain = 64;
		...
		module->patterns = (ibxm_pattern *)calloc( module->num_patterns, sizeof( struct ibxm_pattern ) );
		if( !module->patterns ) {
			ibxm_dispose_module( module );
			return NULL;
		}
		for( idx = 0; idx < module->num_patterns; idx++ ) {
			...
			num_rows = ibxm_data_u16le( data, offset + 5 );
			if( num_rows < 1 ) {
				num_rows = 1;
			}
			pat_data_len = ibxm_data_u16le( data, offset + 7 );
			offset += ibxm_data_u32le( data, offset );
		...
		module->instruments = (ibxm_instrument *)calloc( module->num_instruments + 1, sizeof( struct ibxm_instrument ) );
		if( !module->instruments ) {
			ibxm_dispose_module( module );
			return NULL;
		}
		instrument = &module->instruments[ 0 ];
		instrument->samples = (ibxm_sample *)calloc( 1, sizeof( struct ibxm_sample ) );
		if( !instrument->samples ) {
			ibxm_dispose_module( module );
			return NULL;
		}
		for( ins = 1; ins <= module->num_instruments; ins++ ) {
			...
			num_samples = ibxm_data_u16le( data, offset + 27 );
			instrument->num_samples = ( num_samples > 0 ) ? num_samples : 1;
			instrument->samples = (ibxm_sample *)calloc( instrument->num_samples, sizeof( struct ibxm_sample ) );
			if( !instrument->samples ) {
				ibxm_dispose_module( module );
				return NULL;
			}
			if( num_samples > 0 ) {
				...
				for( point = 0; point < 12; point++ ) {
					...
					point_tick = ( delta_env ? point_tick : 0 ) + ibxm_data_u16le( data, point_offset );
					instrument->vol_env.points_tick[ point ] = point_tick;
					instrument->vol_env.points_ampl[ point ] = ibxm_data_u16le( data, point_offset + 2 );
				}
				point_tick = 0;
				for( point = 0; point < 12; point++ ) {
					...
					instrument->pan_env.points_ampl[ point ] = ibxm_data_u16le( data, point_offset + 2 );
				}
				...
				instrument->vol_fadeout = ibxm_data_u16le( data, offset + 239 );
			}
			offset += ibxm_data_u32le( data, offset );
			...
			for( sam = 0; sam < num_samples; sam++ ) {
				...
				sam_data_bytes = ibxm_data_u32le( data, sam_head_offset );
				sam_loop_start = ibxm_data_u32le( data, sam_head_offset + 4 );
				sam_loop_length = ibxm_data_u32le( data, sam_head_offset + 8 );
...
static struct ibxm_module* module_load_s3m( struct ibxm_data *data, char *message ) {
	...
	if( module ) {
		...
		module->sequence_len = ibxm_data_u16le( data, 32 );
		module->num_instruments = ibxm_data_u16le( data, 34 );
		module->num_patterns = ibxm_data_u16le( data, 36 );
		flags = ibxm_data_u16le( data, 38 );
		version = ibxm_data_u16le( data, 40 );
		module->fast_vol_slides = ( ( flags & 0x40 ) == 0x40 ) || version == 0x1300;
		signed_samples = ibxm_data_u16le( data, 42 ) == 1;
		if( ibxm_data_u32le( data, 44 ) != 0x4d524353 ) {
		...
		for( ins = 1; ins <= module->num_instruments; ins++ ) {
			...
			inst_offset = ibxm_data_u16le( data, module_data_idx ) << 4;
			module_data_idx += 2;
			data_ascii( data, inst_offset + 48, 28, instrument->name );
			if( data_u8( data, inst_offset ) == 1 && ibxm_data_u16le( data, inst_offset + 76 ) == 0x4353 ) {
				sample_offset = ( data_u8( data, inst_offset + 13 ) << 20 )
					+ ( ibxm_data_u16le( data, inst_offset + 14 ) << 4 );
				sample_length = ibxm_data_u32le( data, inst_offset + 16 );
				loop_start = ibxm_data_u32le( data, inst_offset + 20 );
				loop_length = ibxm_data_u32le( data, inst_offset + 24 ) - loop_start;
				...
				tune = ( log_2( ibxm_data_u32le( data, inst_offset + 32 ) ) - log_2( module->c2_rate ) ) * 12;
		...
		for( idx = 0; idx < module->num_patterns; idx++ ) {
			...
			module->patterns[ idx ].data = pattern_data;
			pat_offset = ( ibxm_data_u16le( data, module_data_idx ) << 4 ) + 2;
			row = 0;
			while( row < 64 ) {
...
static struct ibxm_module* module_load_mod( struct ibxm_data *data, char *message ) {
	...
	if( module ) {
		...
		switch( ibxm_data_u16be( data, 1082 ) ) {
			case 0x4b2e: /* M.K. */
			case 0x4b21: /* M!K! */
			case 0x5434: /* FLT4 */
		...
		for( ins = 1; ins <= module->num_instruments; ins++ ) {
			...
			sample_length = ibxm_data_u16be( data, ins * 30 + 12 ) * 2;
			...
			loop_start = ibxm_data_u16be( data, ins * 30 + 16 ) * 2;
			loop_length = ibxm_data_u16be( data, ins * 30 + 18 ) * 2;
			if( loop_start + loop_length > sample_length ) {
...
static void pattern_get_note( struct ibxm_pattern *pattern, int row, int chan, struct note *dest ) {
...
static void channel_init( struct channel *channel, struct ibxm_replay *replay, int idx ) {
...
static void channel_resample( struct channel *channel, int *mix_buf,
	...
	struct ibxm_sample *sample = channel->sample;
...
static void replay_row( struct ibxm_replay *replay ) {
	int idx, count;
	struct note note;
	struct ibxm_pattern *pattern;
	struct channel *channel;
	struct ibxm_module *module = replay->module;
...
static int replay_tick( struct ibxm_replay *replay ) {
...
static int module_init_play_count( struct ibxm_module *module, char **play_count ) {
...
struct ibxm_replay* ibxm_new_replay( struct ibxm_module *module, int sample_rate, int interpolation ) {
	struct ibxm_replay *replay = (ibxm_replay *)calloc( 1, sizeof( struct ibxm_replay ) );
	if( replay ) {
		...
		replay->channels = (channel *)calloc( module->num_channels, sizeof( struct channel ) );
		if( replay->ramp_buf && replay->channels ) {
			ibxm_replay_set_sequence_pos( replay, 0 );
		} else {
			ibxm_dispose_replay( replay );
			replay = NULL;
		}
	}
	return replay;
...
/* Returns the song duration in samples at the current sampling rate. */
int replay_calculate_duration( struct ibxm_replay *replay ) {
...
/* Seek to approximately the specified sample position.
   The actual sample position reached is returned. */
int replay_seek( struct ibxm_replay *replay, int sample_pos ) {
...
static void replay_volume_ramp( struct ibxm_replay *replay, int *mix_buf, int tick_len ) {
...
/* Returns the currently playing pattern in the sequence.*/
int replay_get_sequence_pos( struct ibxm_replay *replay ) {
	return replay->seq_pos;
}

/* Returns the currently playing row in the pattern. */
int replay_get_row( struct ibxm_replay *replay ) {
	return replay->row;
}